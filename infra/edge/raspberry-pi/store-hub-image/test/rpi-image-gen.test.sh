...
#   KITLUY_RIG_UPSTREAM=/tmp/rig bash infra/edge/raspberry-pi/store-hub-image/test/rpi-image-gen.test.sh
#
...
for l in kitluy-store-hub; do
  grep -q "X-Env-Layer-Requires: kitluy-hub-base" "${SRC}/layer/${l}.yaml" 2>/dev/null \
    && ok "${l} builds on kitluy-hub-base" \
    || bad "${l} builds on kitluy-hub-base" "does not require the common base"
done
...
if grep -rIE 'service_role|SUPABASE_SERVICE_ROLE_KEY[[:space:]]*=[[:space:]]*.+|BEGIN [A-Z ]*PRIVATE KEY|PGPASSWORD[[:space:]]*=[[:space:]]*.+' "$SRC" 2>/dev/null \
...
BASE="${SRC}/layer/kitluy-hub-base.yaml"
