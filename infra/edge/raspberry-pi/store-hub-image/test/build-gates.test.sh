...
#
# Run: bash infra/edge/raspberry-pi/store-hub-image/test/build-gates.test.sh
...
for channel in pilot stable; do
  run_build --profile store-hub --channel "$channel" --out "${TMP}/${channel}" --stage-only
...
run_build --profile store-hub --channel invalid --out "${TMP}/inv" --stage-only
...
run_build --profile store-hub --out "${TMP}/term" --stage-only
if [[ $RC -eq 0 ]]; then
  ok "store-hub stages on the internal channel"
else
  bad "store-hub stages on the internal channel" "rc=$RC out=$OUT"
...
run_build --profile store-hub --out "${TMP}/full"
...
run_build --profile store-hub --out "${TMP}/det1" --stage-only
run_build --profile store-hub --out "${TMP}/det2" --stage-only
