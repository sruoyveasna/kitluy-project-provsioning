...
#
#   bash infra/edge/raspberry-pi/store-hub-image/test/image-contents.test.sh [rootfs-path]
#
...
if [[ -z "$ROOTFS" || ! -d "$ROOTFS" ]]; then
  ...
  printf '    bash infra/edge/raspberry-pi/store-hub-image/scripts/build-rpi-image.sh --profile store-hub\n'
