...
> Location: `infra/edge/raspberry-pi/store-hub-image` (previously
> `infra/kitluy-store-hub-image`, moved 2026-09-16). This tree builds the
> **Store Hub image only**; the Pi Terminal image is built from
> `infra/edge/raspberry-pi/pi-terminal-image`. Overview of both Raspberry Pi
...
bash infra/edge/raspberry-pi/store-hub-image/scripts/doctor.sh          # must be green
```
...
```bash
bash infra/edge/raspberry-pi/store-hub-image/scripts/doctor.sh              # host readiness
bash infra/edge/raspberry-pi/store-hub-image/scripts/scan-image-secrets.sh <rootfs> [more...]
bash infra/edge/raspberry-pi/store-hub-image/test/build-gates.test.sh
bash infra/edge/raspberry-pi/store-hub-image/test/systemd-runtime.test.sh
bash infra/edge/raspberry-pi/store-hub-image/test/storage-posture.test.sh
bash infra/edge/raspberry-pi/store-hub-image/test/image-contents.test.sh    # needs a BUILT rootfs
KITLUY_RIG_UPSTREAM=infra/edge/raspberry-pi/store-hub-image/build/upstream \
