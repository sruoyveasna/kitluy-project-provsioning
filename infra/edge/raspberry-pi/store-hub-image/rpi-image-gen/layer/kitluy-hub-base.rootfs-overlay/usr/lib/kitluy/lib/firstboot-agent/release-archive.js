 ...
 */
function headerChecksumValid(block) {
...
export function assertSafeEntryName(name) {
    ...
    if (name.includes("\\"))
        refuse("ENTRY_NAME_UNSAFE_CHARACTER", `${name} contains a backslash`);
    ...
    for (const segment of name.split("/")) {
        if (segment === "..")
            refuse("ENTRY_NAME_TRAVERSAL", name);
    }
}
/**
 ...
 */
export function extractTarGz(archive, root) {
    if (archive.length === 0)
        refuse("ARCHIVE_EMPTY", "zero bytes");
    if (archive.length > ARCHIVE_LIMITS.maxCompressedBytes) {
        refuse("ARCHIVE_COMPRESSED_TOO_LARGE", `${String(archive.length)} bytes exceeds ${String(ARCHIVE_LIMITS.maxCompressedBytes)}`);
    }
    if (ZSTD_MAGIC.every((byte, i) => archive[i] === byte)) {
        refuse("ARCHIVE_COMPRESSION_UNSUPPORTED", "this is a zstd archive; the device runs node 18, which has no in-process zstd. Pack releases as .tar.gz");
    }
    ...
    try {
        ...
        refuse("ARCHIVE_DECOMPRESSION_FAILED", String(error.message ?? error));
    }
    ...
    while (offset + BLOCK <= tar.length) {
        ...
        if (size < 0)
            refuse("ARCHIVE_TRUNCATED", `${name} has an unparseable size field`);
    ...
    if (entries.length === 0)
        refuse("ARCHIVE_EMPTY", "the archive contains no entries");
    return { entries, totalBytes, fileCount, directoryCount };
}
//# sourceMappingURL=release-archive.js.map