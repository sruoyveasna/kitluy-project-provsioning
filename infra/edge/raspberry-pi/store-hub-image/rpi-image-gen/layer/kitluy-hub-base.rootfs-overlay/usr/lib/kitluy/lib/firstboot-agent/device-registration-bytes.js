...
export const DEVICE_REGISTRATION_REQUEST_KIND = "kitluy.device-registration-request.v1";
...
const hasReserved = (value) => RESERVED.some((c) => value.includes(c));
/** Returns null when the request is well-formed. */
export function validateDeviceRegistrationRequest(request) {
    const required = [
        request.assetTag,
    ...
    for (const signal of request.signals) {
        if (typeof signal.signalType !== "string" ||
    ...
    for (const [key, value] of Object.entries(request.installationEvidence)) {
        if (typeof value !== "string")
            return "REGISTRATION_MISSING_FIELD";
        if (hasReserved(key) || hasReserved(value))
            return "REGISTRATION_RESERVED_CHARACTER";
    }
    return null;
}
/** `signalType=signalValue` pairs, sorted by the rendered pair, joined by `;`. */
export function canonicalSignals(signals) {
    return signals
...
export function canonicalInstallationEvidence(evidence) {
 ...
 */
export function deviceRegistrationRequestBytes(request) {
...
//# sourceMappingURL=device-registration-bytes.js.map