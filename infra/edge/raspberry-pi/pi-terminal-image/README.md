...
> Location: `infra/edge/raspberry-pi/pi-terminal-image` (previously
> `infra/kitluy-os-image`, moved 2026-09-16). Overview of both Raspberry Pi
> images: [`../README.md`](../README.md).

**This tree builds the Pi Terminal only.** The Store Hub image is built from
`infra/edge/raspberry-pi/store-hub-image` (owner decision 2026-08-13: two devices, two
...
```bash
sudo bash infra/edge/raspberry-pi/pi-terminal-image/scripts/setup-ubuntu-arm64-builder.sh
bash infra/edge/raspberry-pi/pi-terminal-image/scripts/doctor.sh          # must be green
```
...
`infra/edge/raspberry-pi/store-hub-image/README.md` and apply unchanged.
...
KITLUY_DEV_SSH_PUBKEY=$HOME/.ssh/id_ed25519.pub KITLUY_DEV_SUDO=1 \
  bash infra/edge/raspberry-pi/pi-terminal-image/scripts/build-rpi-image.sh --profile pi-terminal \
...
bash infra/edge/raspberry-pi/pi-terminal-image/test/build-gates.test.sh
bash infra/edge/raspberry-pi/pi-terminal-image/test/systemd-runtime.test.sh
bash infra/edge/raspberry-pi/pi-terminal-image/test/environment-gating.test.sh
KITLUY_RIG_UPSTREAM=infra/edge/raspberry-pi/pi-terminal-image/build/upstream \
  bash infra/edge/raspberry-pi/pi-terminal-image/test/rpi-image-gen.test.sh
bash infra/edge/raspberry-pi/pi-terminal-image/test/image-contents.test.sh          # needs a BUILT rootfs
bash infra/edge/raspberry-pi/pi-terminal-image/scripts/scan-image-secrets.sh <rootfs>
```
