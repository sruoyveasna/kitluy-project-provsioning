...
export const CROCKFORD_BASE32 = {
    alphabet: "0123456789ABCDEFGHJKMNPQRSTVWXYZ",
    length: 8,
};
export const emptyCodeEntry = { value: "", complete: false };
/**
 ...
 */
export function normalizeCodeChar(raw, format = CROCKFORD_BASE32) {
    if (raw.length !== 1)
        return null;
 ...
 */
export function reduceCodeEntry(state, action, format = CROCKFORD_BASE32) {
    switch (action.kind) {
        case "key": {
            if (state.value.length >= format.length)
                return state;
 ...
 */
export function feedCodeEntry(state, chars, format = CROCKFORD_BASE32) {
    let next = state;
...
//# sourceMappingURL=code-entry.js.map