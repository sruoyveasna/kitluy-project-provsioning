...
electron_1.contextBridge.exposeInMainWorld("kitluyShell", {
    ...
    joinNetwork: (ssidHex, passphrase) => electron_1.ipcRenderer.invoke(NETWORK_JOIN_CHANNEL, ssidHex, passphrase),
    forgetNetwork: (ssidHex) => electron_1.ipcRenderer.invoke(NETWORK_FORGET_CHANNEL, ssidHex),
    getBrightness: () => electron_1.ipcRenderer.invoke(BRIGHTNESS_GET_CHANNEL),
    setBrightness: (percent) => electron_1.ipcRenderer.invoke(BRIGHTNESS_SET_CHANNEL, percent),
    getDeviceInfo: () => electron_1.ipcRenderer.invoke(DEVICE_INFO_CHANNEL),
    getPrinter: () => electron_1.ipcRenderer.invoke(PRINTER_GET_CHANNEL),
    savePrinter: (config) => electron_1.ipcRenderer.invoke(PRINTER_SAVE_CHANNEL, config),
    testPrinter: () => electron_1.ipcRenderer.invoke(PRINTER_TEST_CHANNEL),
});
