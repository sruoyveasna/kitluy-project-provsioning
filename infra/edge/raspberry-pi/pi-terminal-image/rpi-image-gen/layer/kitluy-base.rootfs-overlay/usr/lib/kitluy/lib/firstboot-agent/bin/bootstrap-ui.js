 ...
 * it knows about.
 */
import { hasDefaultRoute, readBootstrapState } from "../bootstrap-state.js";
import { readImageEnv } from "../image-env.js";
import { assetTagFromFingerprint, readRegistrationState, registrationHeadline, registrationPhaseLabel, } from "../registration-state.js";
...
export const PAIRING_UNAVAILABLE_LINE = "Pairing is not available in this build; Store assignment arrives with a later release.";
export function render(bootstrap, registration, options = {}) {
    const now = options.now ?? new Date();
    const network = bootstrap !== null
    ...
    const assetTag = registration?.keyFingerprint === undefined
    ...
    const noteRows = registration === null
        ? []
...
export async function main() {
    for (;;) {
        ...
        process.stdout.write(render(readBootstrapState(), readRegistrationState(), {
