 ...
 */
export function findBacklight(root = BACKLIGHT_ROOT) {
    try {
        const entries = readdirSync(root).sort();
        for (const name of entries) {
            if (existsSync(`${root}/${name}/max_brightness`))
                return `${root}/${name}`;
        }
    }
    catch {
...
export function writeBrightnessPercent(dir, percent) {
    if (dir === null)
        throw new Error("no backlight");
    const max = Number(readFileSync(`${dir}/max_brightness`, "utf8").trim());
    if (!Number.isFinite(max) || max <= 0)
        throw new Error("no usable backlight range");
...
export function forgetNetwork(ssidHex, configPath = WPA_CONFIG_PATH) {
...
export function buildDeps() {
    ...
    return {
        ...
        ...(backlight === null
            ? {}
            : { setBrightness: async (percent) => writeBrightnessPercent(backlight, percent) }),
    };
}
/** Wire one accepted connection to the verb handler. */
export function attach(socket, deps) {
    let buffer = "";
    let poisoned = false;
...
//# sourceMappingURL=device-config-broker.js.map