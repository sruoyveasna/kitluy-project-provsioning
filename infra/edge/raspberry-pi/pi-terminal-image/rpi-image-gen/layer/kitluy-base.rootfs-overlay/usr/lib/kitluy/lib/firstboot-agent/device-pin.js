...
const SEAL_VERSION = "kitluy.device-pin.sealed.v1";
...
export function readDevicePinPosture(paths = DEFAULT_DEVICE_PIN_PATHS) {
    ...
    try {
        ...
        if (parsed.state !== "sealed" && parsed.state !== "registered")
            return absent;
        return {
            schema: "kitluy.device-pin-posture.v1",
            state: parsed.state,
 ...
 */
export function sealDevicePin(input, paths = DEFAULT_DEVICE_PIN_PATHS, now = new Date()) {
    ...
    if (current.state === "registered")
        return { ok: false, code: "PIN_ALREADY_REGISTERED" };
...
export function unsealDevicePin(paths = DEFAULT_DEVICE_PIN_PATHS) {
    if (!existsSync(paths.sealed))
        return null;
    try {
        const record = JSON.parse(readFileSync(paths.sealed, "utf8"));
        if (record.schema !== SEAL_VERSION)
            return null;
