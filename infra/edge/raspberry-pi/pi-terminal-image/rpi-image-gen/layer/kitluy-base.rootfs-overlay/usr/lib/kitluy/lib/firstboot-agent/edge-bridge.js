...
export const FORWARDED_HEADERS = ["idempotency-key", "x-kitluy-session-id"];
function refusal(status, code, message, retryable) {
...
export function matchBridgeRoute(method, url) {
    let parsed;
    try {
    ...
    if (route === undefined) {
        return {
            ok: false,
            response: refusal(404, "EDGE_BRIDGE_ROUTE_NOT_ALLOWED", "the edge bridge does not forward this route", false),
        };
    }
    ...
    if (keys.some((key) => !allowed.has(key)) || new Set(keys).size !== keys.length) {
        return {
            ok: false,
            response: refusal(400, "EDGE_BRIDGE_REQUEST_INVALID", "the route does not take that query string", false),
        };
    }
 ...
 * classify.
 */
export async function handleBridgeRequest(deps, request) {
    ...
    if (request.method !== "GET") {
        ...
        try {
            body =
                request.body.length === 0 ? {} : JSON.parse(request.body.toString("utf8"));
        }
        catch {
            return refusal(400, "EDGE_BRIDGE_REQUEST_INVALID", "the request body is not JSON", false);
        }
        if (typeof body !== "object" || body === null || Array.isArray(body)) {
            return refusal(400, "EDGE_BRIDGE_REQUEST_INVALID", "the request body is not an object", false);
        }
    }
    ...
    if (!credentials.available) {
        return refusal(503, "ACTIVATION_REQUIRED", "this terminal holds no operational certificate yet", true);
    }
    const endpoint = deps.pinnedEndpoint();
    if (endpoint === null) {
        return refusal(503, "EDGE_NOT_CONNECTED", "no verified Store Hub endpoint yet; the terminal edge is still looking", true);
    }
    const headers = {};
    ...
    try {
        ...
        return refusal(502, mismatch ? "HUB_CERT_MISMATCH" : "EDGE_HUB_UNREACHABLE", mismatch
            ? "the Store Hub presented a certificate other than the pinned one"
            : "the Store Hub did not answer over the pinned mutual-TLS link", !mismatch);
    }
...
export function groupId(name, groupFile = "/etc/group") {
    try {
        for (const line of readFileSync(groupFile, "utf8").split("\n")) {
            const [group, , gid] = line.split(":");
            if (group === name && gid !== undefined && /^\d+$/u.test(gid))
                return Number(gid);
        }
    }
    catch {
 ...
 * everyone.
 */
export async function startEdgeBridge(deps, options = {}) {
    ...
    const server = createServer((req, res) => {
        void readRequest(req).then(async (body) => {
            if (body === null) {
                send(res, refusal(413, "EDGE_BRIDGE_REQUEST_INVALID", "the request body is too large", false));
                return;
            }
            send(res, await handleBridgeRequest(deps, {
                method: req.method ?? "",
                url: req.url ?? "",
                headers: req.headers,
                body,
    ...
    if (gid === null) {
        ...
        deps.log?.(`bridge listening on ${socketPath} (group ${options.group ?? BRIDGE_CLIENT_GROUP} not found; root only)`);
        return server;
    }
    try {
        ...
        deps.log?.(`bridge listening on ${socketPath} for group ${options.group ?? BRIDGE_CLIENT_GROUP}`);
    }
    catch {
        chmodSync(socketPath, 0o600);
...
//# sourceMappingURL=edge-bridge.js.map