...
log()  { printf '[kitluy-os-image] %s\n' "$*" >&2; }
warn() { printf '[kitluy-os-image] WARNING: %s\n' "$*" >&2; }
die()  { printf '[kitluy-os-image] REFUSED: %s\n' "$*" >&2; exit 1; }
...
readonly KITLUY_KNOWN_PROFILES="store-hub pi-terminal"
