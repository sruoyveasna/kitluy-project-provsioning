...
function encodeName(name) {
    ...
    for (const part of parts) {
        ...
        if (label.length > 63)
            throw new Error("mDNS label too long");
...
function parseRecords(buffer) {
    ...
    for (let i = 0; i < total; i += 1) {
        ...
        if (rdataOffset + rdlength > buffer.length)
            break;
        records.push({
            name,
            type,
...
export function udpMulticastPort() {
...
export function buildServiceQuery(serviceType = EDGE_DISCOVERY_SERVICE_TYPE) {
...
export function collectCandidates(messages, serviceType = EDGE_DISCOVERY_SERVICE_TYPE) {
...
//# sourceMappingURL=edge-mdns.js.map