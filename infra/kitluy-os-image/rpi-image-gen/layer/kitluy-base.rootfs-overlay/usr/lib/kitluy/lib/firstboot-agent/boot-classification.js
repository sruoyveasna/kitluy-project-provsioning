...
import { classifyBoot, DEFAULT_USER_MESSAGES, } from "./boot-classification-contract.js";
...
import { REGISTRATION_STATE_PATH, readRegistrationState, } from "./registration-state.js";
 ...
 */
export function readMediaClaims(paths = {}) {
    const deviceClass = readImageEnv("KITLUY_DEVICE_CLASS", paths.etcRoot);
    if (deviceClass !== "store_hub" && deviceClass !== "terminal")
        return null;
    const imageDeviceClass = deviceClass;
    ...
    const imageEnvironment = environment;
    ...
    const deviceRecordId = uuidOrUndefined(manifest?.deviceRecordId) ??
...
export function registrationOutcomeFor(phase) {
    switch (phase) {
 ...
 */
export function readHubStoragePosture(devRoot = "/dev") {
    let entries;
    try {
    ...
    try {
        if (readdirSync(join(devRoot, "mapper")).includes(HUB_DATA_MAPPER))
            return "opened";
    }
    catch {
...
export function bootSignalsFrom(hardware) {
    const signals = [];
    const add = (signalType, value) => {
...
export function claimsDigest(media) {
    const sorted = Object.fromEntries(Object.entries(media).sort(([a], [b]) => (a < b ? -1 : a > b ? 1 : 0)));
...
export function shopDecisionFrom(value) {
    if (value === null || typeof value !== "object")
        return null;
    const v = value;
    if (typeof v.classification !== "string" ||
        !CLASSIFICATIONS.includes(v.classification) ||
        typeof v.nextAction !== "string" ||
        !NEXT_ACTIONS.includes(v.nextAction) ||
    ...
    return {
        ...
        userMessageKey: v.userMessageKey,
        retryAutomatically: v.retryAutomatically,
        servesLocally: v.servesLocally,
    };
}
export function readBootClassificationState(path = BOOT_CLASSIFICATION_STATE_PATH) {
    try {
        const raw = JSON.parse(readFileSync(path, "utf8"));
        ...
        if (current === null)
            return null;
        const c = raw.current;
        const last = raw.lastCloudAnswer;
        const lastCloudAnswer = last !== undefined &&
            typeof last.classification === "string" &&
            CLASSIFICATIONS.includes(last.classification) &&
...
export function writeBootClassificationState(state, path = BOOT_CLASSIFICATION_STATE_PATH) {
...
export async function classifyThisBoot(input) {
    ...
    const confirmedReadyOnThisBoard = last !== undefined &&
