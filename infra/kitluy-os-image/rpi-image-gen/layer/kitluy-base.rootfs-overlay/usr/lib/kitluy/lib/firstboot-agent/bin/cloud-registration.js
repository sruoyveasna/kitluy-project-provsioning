...
import { createHttpRegistrationClient, fingerprintFromPem, } from "../adapters/http-registration-client.js";
...
import { writeRegistrationState, } from "../registration-state.js";
 ...
 */
function readRegistrationUrl(etcRoot) {
 ...
 */
function readHardwareProfileKey(etcRoot) {
    return readImageEnv("KITLUY_HARDWARE_PROFILE_KEY", etcRoot);
}
function nowIso() {
    return new Date().toISOString();
}
function state(phase, rest = {}) {
 ...
 */
export async function runRegistrationPass(options = {}) {
    ...
    if (registrationUrl === undefined) {
        ...
        return persist(state("NOT_REGISTERED", {
    ...
    switch (result.kind) {
        case "pending":
            return persist(state("AWAITING_APPROVAL", {
        ...
        case "trust_review": {
            ...
            return persist(state(contained ? "CONTAINED" : "TRUST_REVIEW_REQUIRED", {
        ...
        case "refused":
            return persist(state(result.code === "REGISTRATION_UNREACHABLE" ? "UNREACHABLE" : "NOT_REGISTERED", {
...
function report(current) {
    process.stdout.write(`event=kitluy.device.registration phase=${current.phase} device=${current.deviceId ?? ""} detail=${JSON.stringify(current.detail ?? "")}\n`);
}
...
//# sourceMappingURL=cloud-registration.js.map