...
export function readPrinterConfig(path = PRINTER_CONFIG_PATH) {
    try {
...
export function writePrinterConfig(config, path = PRINTER_CONFIG_PATH) {
 ...
 */
export function testReceiptBytes(now, label) {
    const ESC = 0x1b;
    const GS = 0x1d;
    const lines = [
        "KitLuy Terminal",
...
export function printTest(config, options = {}) {
    ...
    return new Promise((resolve) => {
        ...
        socket.on("connect", () => {
            socket.end(testReceiptBytes(options.now ?? new Date(), config.label), () => finish({ ok: true }));
        });
