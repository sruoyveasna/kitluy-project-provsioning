...
for profile in store-hub pi-terminal; do
...
# ---------------------------------------------------------------------------
for overlay_unit in "${LAYER_DIR}"/kitluy-base.rootfs-overlay/etc/systemd/system/kitluy-*.service \
                    "${LAYER_DIR}"/kitluy-pi-terminal.rootfs-overlay/etc/systemd/system/kitluy-*.service; do
  [[ -e "$overlay_unit" ]] || continue
  name="$(basename "$overlay_unit")"
