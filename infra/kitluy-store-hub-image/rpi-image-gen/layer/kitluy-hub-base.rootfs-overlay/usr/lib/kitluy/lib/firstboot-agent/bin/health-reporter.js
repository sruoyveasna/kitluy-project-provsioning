...
export function buildHealthSummary(now = new Date()) {
...
export async function main() {
    for (;;) {
        ...
        process.stdout.write(`event=kitluy.health.beat class=${summary.deviceClass} health=${summary.health} assignment=${summary.assignmentState} agent=${summary.agentVersion} image=${summary.imageVersion ?? "unknown"} device=${summary.deviceLabel ?? "unknown"}\n`);
        const state = readBootstrapState();
        if (state !== null)
