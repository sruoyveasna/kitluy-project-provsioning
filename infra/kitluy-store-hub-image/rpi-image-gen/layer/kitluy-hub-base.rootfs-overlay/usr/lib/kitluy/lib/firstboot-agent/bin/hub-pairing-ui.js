...
export function interpret(attempt, deviceRecordId) {
    ...
    switch (attempt.result) {
        ...
        case "CODE_REFUSED":
            return {
                message: "That pairing code was not accepted for this device. Check it and try again.\n" +
        ...
        case "ALREADY_ASSIGNED":
            return {
                message: "This Hub is already assigned to a Store in KitLuy, so a pairing code cannot be redeemed for it.\n" +
 ...
 */
export function assertStdinIsFree(input) {
    const listeners = input.listenerCount("data") + input.listenerCount("keypress");
    if (listeners > 0) {
        throw new Error("KLUY-CONSOLE-STDIN-BUSY: refusing to read a secret while another reader is attached to " +
