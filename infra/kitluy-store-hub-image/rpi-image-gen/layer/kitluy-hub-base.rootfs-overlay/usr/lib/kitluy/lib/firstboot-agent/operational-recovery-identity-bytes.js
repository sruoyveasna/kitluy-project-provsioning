...
export const RECOVERY_IDENTITY_PROOF_KIND = "kitluy.opcert-recovery-identity.v1";
/**
...
export function recoveryIdentityProofBytes(identityPublicKeyFingerprint, csrBytes) {
    if (!HEX64.test(identityPublicKeyFingerprint)) {
        throw new Error("KLUY-RECOVERY-IDENTITY-MALFORMED: the identity key fingerprint must be a lowercase sha-256 hex digest");
    }
    const csrDigest = createHash("sha256").update(Buffer.from(csrBytes)).digest("hex");
    return new Uint8Array(Buffer.from(`${RECOVERY_IDENTITY_PROOF_KIND}\n${identityPublicKeyFingerprint}\n${csrDigest}`, "utf8"));
}
 ...
 */
export function fileRecoveryIdentitySigner(keyPath = DEVICE_IDENTITY_PRIVATE_KEY_PATH) {
    return (csrBytes) => {
        let privateKey;
        try {
            ...
            throw new RecoveryIdentityError(`the device identity key at ${keyPath} could not be read (detail withheld)`);
        }
        ...
        const fingerprint = createHash("sha256")
            ...
            .digest("hex");
        const signature = cryptoSign(null, Buffer.from(recoveryIdentityProofBytes(fingerprint, csrBytes)), privateKey);
        return { identityPublicKeyPem, identityProofBase64: signature.toString("base64") };
    };
}
//# sourceMappingURL=operational-recovery-identity-bytes.js.map