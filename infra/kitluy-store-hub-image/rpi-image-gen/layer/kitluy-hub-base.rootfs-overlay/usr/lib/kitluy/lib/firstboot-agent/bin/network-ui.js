 ...
 */
import { isCloudReachable, joinNetwork, readNetworkStatus, scanAccessPoints, waitForWirelessCarrier, } from "../network.js";
const CLEAR_SCREEN = "[2J[H";
/** Signal strength as three words rather than dBm, which means nothing in a shop. */
export function strengthLabel(signal) {
    if (signal >= -60)
        return "strong";
    if (signal >= -75)
        return "medium";
...
export function renderNetworks(points) {
    if (points.length === 0) {
    ...
    const rows = points.slice(0, 9).map((p, i) => {
        ...
        return `  ${i + 1}. ${name.padEnd(26)}${strengthLabel(p.signal).padEnd(8)}${p.secured ? "secured" : "open"}`;
    });
    return [
        "",
 ...
 */
export async function awaitNetwork(io, options = {}) {
    const status = options.status ?? (() => readNetworkStatus());
    const scan = options.scan ?? (() => scanAccessPoints());
    const join = options.join ?? joinNetwork;
    const reachable = options.reachable ?? ((url) => isCloudReachable(url));
    const settle = options.settle ?? (() => waitForWirelessCarrier());
    ...
    for (let round = 0; round < maxRounds; round += 1) {
        ...
        if (now.hasLink && (await reachable(baseUrl)))
            return true;
...
//# sourceMappingURL=network-ui.js.map