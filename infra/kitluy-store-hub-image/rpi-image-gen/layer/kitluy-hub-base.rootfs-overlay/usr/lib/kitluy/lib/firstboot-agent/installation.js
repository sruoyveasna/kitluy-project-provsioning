...
function writeInstallation(record, path) {
...
export function readInstallation(path = INSTALLATION_STATE_PATH) {
    try {
        ...
        if (!UUID_PATTERN.test(candidate.installationId))
            return null;
        return parsed;
    }
    catch {
 ...
 */
export function loadOrCreateInstallation(options = {}) {
