...
import { downloadAndVerify } from "./release-artifact.js";
import { evaluateAssignment, withAcceptedSequence, } from "./release-assignment.js";
import { activate, beginStaging, commit, promoteStaged, pruneReleases, readJournal, recordRefusal, reconcile, rollback, writeJournal, } from "./release-store.js";
import { findReleaseAcceptanceRefusal, verifyReleaseManifestSignature, } from "./release-verify.js";
 ...
 */
export async function runInstallPass(deps) {
    const { paths } = deps;
    ...
    if (reconciled.action === "RESUME_HEALTH_GATE") {
        ...
        if (target !== null) {
            return runHealthGate(deps, target, reconciled.journal.committedVersion ?? "unknown", sleep, now);
        }
    }
    ...
    try {
        ...
        const refusal = error instanceof ArchiveRefusedError ? error.refusal : "ARCHIVE_EXTRACTION_FAILED";
        recordRefusal(paths, releaseId, `${refusal}: ${String(error.message ?? error)}`);
        return { outcome: "REFUSED", code: refusal, detail: String(error.message ?? error) };
    }
    ...
    writeDurable(join(incoming, "manifest.json"), `${JSON.stringify({ manifest, envelope, verifiedBy: verified.keyId, keyVersion: verified.keyVersion }, null, 2)}\n`, 0o644);
    ...
    try {
        ...
        const journal = rollback(paths, releaseId, `the unit would not restart: ${String(error.message ?? error)}`);
        return {
            outcome: "ROLLED_BACK",
            releaseId,
 ...
 */
function preflightPayload(incoming) {
    const packageJson = join(incoming, "payload", "package.json");
    if (!existsSync(packageJson))
        return "PAYLOAD_NO_PACKAGE_JSON";
    try {
        const parsed = JSON.parse(readFileSync(packageJson, "utf8"));
        if (typeof parsed.main !== "string" || parsed.main === "")
            return "PAYLOAD_NO_MAIN";
        if (!existsSync(join(incoming, "payload", parsed.main)))
            return "PAYLOAD_MAIN_MISSING";
    }
    catch {
 ...
 */
async function runHealthGate(deps, releaseId, version, sleep, now) {
    const started = now();
    let consecutive = 0;
