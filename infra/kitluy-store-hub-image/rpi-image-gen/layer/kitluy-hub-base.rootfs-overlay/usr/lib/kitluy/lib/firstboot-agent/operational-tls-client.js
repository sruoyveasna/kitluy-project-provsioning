...
import { ensureOperationalKey, withOperationalPrivateKey, OperationalKeyError, } from "./operational-key.js";
 ...
 */
export async function ensureOperationalCertificate(deps) {
    ...
    if (requestState !== null && requestState.publicKeyFingerprint !== key.publicKeyFingerprint) {
        ...
        return {
            kind: "blocked",
            detail: "the stored issuance request was made for a different operational key; " +
    ...
    try {
        ...
        return {
            kind: "blocked",
            detail: error instanceof OperationalKeyError ? error.message : "the request could not be signed",
            phase: currentPhase(paths),
        };
    }
    ...
    commitAdoption({ certificatePem: response.certificatePem, chainPem: response.chainPem, manifest }, paths);
 ...
 */
export function x509SerialForCredentialSerial(credentialSerial) {
...
//# sourceMappingURL=operational-tls-client.js.map