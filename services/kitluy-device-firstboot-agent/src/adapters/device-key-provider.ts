...
import type { KeyProvider } from "../identity.js";
...
export class FileKeyProvider implements KeyProvider {
  ...
  async generateKeyPair(): Promise<{ publicKeyPem: string; privateKeyHandle: string }> {
   ...
   */
  async verifyKeyUsable(privateKeyHandle: string): Promise<boolean> {
    try {
      const pem = readFileSync(privateKeyHandle, "utf8");
      const privateKey = createPrivateKey(pem);
      if (privateKey.asymmetricKeyType !== KEY_ALGORITHM) return false;
   ...
   */
  async signPayload(privateKeyHandle: string, payload: Uint8Array): Promise<Uint8Array> {
    let pem: string;
    try {
