...
import type { HardwareProbe, HardwareSignals } from "../identity.js";

export interface LinuxHardwareProbeOptions {
  /** Overridable so tests need no real /sys or /proc. */
  readonly sysRoot?: string;
  readonly procRoot?: string;
}

export class LinuxHardwareProbe implements HardwareProbe {
  readonly #sys: string;
  readonly #proc: string;

  constructor(options: LinuxHardwareProbeOptions = {}) {
    this.#sys = options.sysRoot ?? "/sys";
    this.#proc = options.procRoot ?? "/proc";
  }

  async collect(): Promise<HardwareSignals> {
    // Built by explicit assignment rather than object spread so a signal is
    // present only when it was actually read.
    const signals: {
      -readonly [K in keyof HardwareSignals]: HardwareSignals[K];
    } = {};

    const mac = this.#macAddress();
    if (mac !== undefined) signals.macAddress = mac;

    const board = this.#deviceTreeSerial() ?? this.#cpuinfoSerial();
    if (board !== undefined) signals.boardSerial = board;
    ...
    if (soc !== undefined) signals.socSerial = soc;

    const storage = this.#storage();
    if (storage.serial !== undefined) signals.storageSerial = storage.serial;
    if (storage.model !== undefined) signals.storageModel = storage.model;
  ...
  #macAddress(): string | undefined {
    ...
    for (const name of interfaces) {
      ...
      if (address !== undefined && address !== "00:00:00:00:00:00") return address;
    }
    return undefined;
  }

  #deviceTreeSerial(): string | undefined {
  ...
  #cpuinfoSerial(): string | undefined {
  ...
  #storage(): { serial?: string; model?: string } {
    let blocks: string[];
    try {
    ...
    for (const block of ordered) {
      ...
      if (serial !== undefined || model !== undefined) {
        const out: { serial?: string; model?: string } = {};
