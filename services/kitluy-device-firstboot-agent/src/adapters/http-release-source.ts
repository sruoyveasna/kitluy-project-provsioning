...
import type { ArtifactSource } from "../release-artifact.js";
import type { AssignmentSource, ReleaseAssignment } from "../release-assignment.js";
import type { ReleaseSignatureEnvelope, SignedReleaseManifestBody } from "../release-verify.js";

export interface HttpReleaseSourceOptions {
  /** `KITLUY_RELEASE_SOURCE` from /etc/kitluy/release.env. */
  readonly baseUrl: string;
  /** The device's own asset tag — what it calls itself, never a secret. */
  /**
   * How this device names ITSELF to the authority — the cloud-issued device id
   * from the registration state, never a locally derived tag. See the note in
   * `release-runtime.ts`: a derived tag stops matching the cloud's after the
...
function fetchRaw(
  url: string,
  options: { readonly timeoutMs: number; readonly range?: string; readonly maxBytes: number },
): Promise<RawResponse> {
  return new Promise((resolve, reject) => {
    ...
    const req = send(
      ...
      (res) => {
        ...
        if (status >= 300 && status < 400) {
          res.resume();
          reject(
            new ReleaseSourceError(
              `the release source answered a redirect (${String(status)}); a byte transport does not get to say where to look`,
            ),
          );
          return;
        }
    ...
    req.on("timeout", () => {
      ...
      reject(
        new ReleaseSourceError(
          `the release source did not answer within ${String(options.timeoutMs)}ms`,
        ),
      );
    });
    req.on("error", (error) => {
      reject(new ReleaseSourceError(String((error as Error).message ?? error)));
    });
    req.end();
  });
}
 ...
 */
export function parseAssignment(payload: unknown): ReleaseAssignment | null {
  if (typeof payload !== "object" || payload === null) {
    throw new ReleaseSourceError("the assignment response is not an object");
  }
  const assignment = (payload as { assignment?: unknown }).assignment;
  ...
  if (assignmentId === null || deviceId === null || environment === null) {
    throw new ReleaseSourceError(
      "the assignment lacks its binding (assignmentId, deviceId, environment)",
    );
  }
  ...
  const a = rawAssignmentEnvelope as Record<string, unknown>;
  const assignmentEnvelope: ReleaseSignatureEnvelope = {
  ...
  if (asString(a["algorithm"]) !== "ed25519") {
    throw new ReleaseSourceError(
      `unsupported assignment signature algorithm ${String(a["algorithm"])}`,
    );
  }
  return {
    assignmentId,
...
export function createHttpReleaseSource(
  options: HttpReleaseSourceOptions,
): AssignmentSource & ArtifactSource {
  ...
  return {
    ...
    async fetchAssignment(): Promise<ReleaseAssignment | null> {
      ...
      if (response.status !== 200) {
        throw new ReleaseSourceError(
          `the assignment authority answered ${String(response.status)}`,
        );
      }
      let payload: unknown;
      try {
    ...
    async fetchChunk(releaseId: string, offset: number, maxBytes: number) {
