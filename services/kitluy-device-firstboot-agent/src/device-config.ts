...
import {
  ...
  type DevicePinRefusal,
} from "./device-pin.js";

/** A scanned network as it crosses the socket. `ssidBytes` is a Buffer and does not. */
export interface AccessPointDto {
  /** Printable, control bytes already replaced. For DISPLAY only. */
  readonly ssid: string;
  /** The real bytes, hex-encoded. This is what a join must quote back. */
...
export type DeviceConfigRequest =
  | { readonly verb: "network.status" }
  | { readonly verb: "network.scan" }
...
export interface DeviceConfigDeps {
  ...
  readonly scan?: () => Promise<readonly AccessPoint[]>;
  readonly join?: (ssidHex: string, psk?: string) => Promise<WifiJoinOutcome>;
  ...
  /** Create the Terminal PIN on the Store Hub (through the bridge). Injected in tests. */
...
const PIN_REFUSAL_TEXT: Record<DevicePinRefusal, string> = {
 ...
 */
export function parseRequest(line: string): DeviceConfigRequest | DeviceConfigResponse {
  ...
  const verb = (raw as { verb?: unknown }).verb;
  ...
  if (verb === "network.join" || verb === "network.forget") {
    const ssidHex = (raw as { ssidHex?: unknown }).ssidHex;
    if (
      typeof ssidHex !== "string" ||
      ssidHex === "" ||
      !HEX.test(ssidHex) ||
      ssidHex.length % 2 !== 0
    ) {
    ...
    if (verb === "network.forget") return { verb, ssidHex };
    const psk = (raw as { psk?: unknown }).psk;
  ...
  return { verb } as DeviceConfigRequest;
}

export function toAccessPointDto(ap: AccessPoint): AccessPointDto {
  return {
  ...
  try {
    switch (request.verb) {
      ...
      case "display.getBrightness": {
        if (deps.getBrightness === undefined) return { ok: true, data: { percent: null } };
