...
export const OPERATIONAL_KEY_TYPE = "rsa" as const;
export const OPERATIONAL_KEY_BITS = 2048 as const;
...
export interface OperationalKeyHandle {
  /** The PUBLIC half. Safe to send, safe to log. */
  readonly publicKeyPem: string;
  /** SHA-256 over the DER SPKI, lowercase hex — the repository's one spelling. */
  readonly publicKeyFingerprint: string;
  /** True when this call created the key rather than reading an existing one. */
...
function assertKeyMode(path: string): void {
  let mode: number;
  try {
  ...
  if ((mode & 0o077) !== 0) {
    throw new OperationalKeyError(
      `${path} is mode ${mode.toString(8)}; the operational key must be owner-only (0600)`,
    );
  }
}

/** Parse a stored key, or refuse. Never regenerates over a bad one. */
function loadExisting(path: string): { key: KeyObject; publicKeyPem: string } {
  assertKeyMode(path);
  let pem: string;
  try {
  ...
  if (!pem.includes("PRIVATE KEY")) {
    ...
    throw new OperationalKeyError(
      `${path} is not a PEM private key; it may be a partial write. ` +
  ...
  try {
    ...
    throw new OperationalKeyError(
      `${path} could not be parsed as a private key; the underlying reason is withheld ` +
        "because it can contain key material. Refusing to replace it.",
    );
  }
  if (key.asymmetricKeyType !== OPERATIONAL_KEY_TYPE) {
    throw new OperationalKeyError(
      `${path} holds a ${key.asymmetricKeyType ?? "unknown"} key; this profile requires ` +
  ...
  if (bits !== OPERATIONAL_KEY_BITS) {
    throw new OperationalKeyError(
      `${path} holds an RSA-${bits ?? "unknown"} key; this profile requires ` +
        `${OPERATIONAL_KEY_ALGORITHM}. Refusing to replace it.`,
    );
  }
  return {
 ...
 * have to guess about.
 */
export function ensureOperationalKey(path = OPERATIONAL_KEY_PATH): OperationalKeyHandle {
  if (existsSync(path)) {
  ...
  try {
    ...
    if (error instanceof OperationalKeyError) throw error;
    throw new OperationalKeyError(
      `could not create ${path}; the underlying reason is withheld because it can ` +
 ...
 */
export function withOperationalPrivateKey<T>(
  use: (privateKey: KeyObject) => T,
  path = OPERATIONAL_KEY_PATH,
): T {
  ...
  try {
    ...
    if (error instanceof OperationalKeyError) throw error;
    throw new OperationalKeyError(
      "the operational key could not complete the requested operation; the underlying " +
