...
import {
  checkRecord,
  isSignedDiscoveryPayload,
  EDGE_LAN_PORT,
  type DiscoveryVerdict,
} from "./edge-discovery-record.js";
import { discoverEdgeCandidates, type EdgeCandidate } from "./edge-mdns.js";
import { pairWithHub, type PairingOutcome } from "./edge-pairing.js";
import {
  edgeRequest,
...
export type EdgePhase =
  /** No operational certificate yet — the terminal is not activated. */
  | "NOT_ACTIVATED"
  /** Nothing answered on the LAN. */
  | "NO_HUB_FOUND"
  /** A Hub answered but its record was refused. */
  | "HUB_REFUSED"
  /** The Hub does not hold this terminal (the BLK-006 projection gap). */
  | "NOT_RECOGNIZED"
  /** The Hub holds this terminal but will not pair it (no profile, or refused). */
  | "PAIRING_REFUSED"
  /**
   * Paired and answering, but a Hub-side dependency is unavailable for at least
   * one bootstrap read. Its own phase because it is a DIFFERENT thing from a
   * refusal: the link is good and the shop is blocked on the Hub, not on this
   * terminal. Collapsing it into HUB_REFUSED would send someone to look here.
   */
  | "DEGRADED"
  /** Connected, paired, and the Hub answered every bootstrap read. */
  | "SERVING";
...
export interface EdgeAttemptOptions {
  readonly environment: string;
  readonly operationalDir?: string;
  readonly statusPath?: string;
  readonly lastEndpointPath?: string;
  ...
  /**
   * The profiles the cloud assigned this terminal, most preferred first. The
   * Hub pairs a terminal INTO one profile, so with none known there is nothing
   * to ask for and pairing is not attempted rather than guessed at.
   */
  ...
  readonly requestFn?: typeof edgeRequest;
}
...
function readLastEndpoint(path: string): EdgeCandidate | null {
  try {
    const parsed = JSON.parse(readFileSync(path, "utf8")) as Partial<EdgeCandidate>;
...
function refusalResult(body: unknown): string {
  const envelope = body as {
    readonly error?: { readonly code?: string; readonly details?: { readonly result?: string } };
  } | null;
  return envelope?.error?.details?.result ?? envelope?.error?.code ?? "UNKNOWN";
}

/** One full attempt. Returns the status it also wrote. */
export async function runEdgeAttempt(options: EdgeAttemptOptions): Promise<EdgeStatus> {
  ...
  const publish = (status: EdgeStatus): EdgeStatus => {
...
async function attemptCandidate(
  ...
  options: EdgeAttemptOptions,
  call: typeof edgeRequest,
  now: () => Date,
): Promise<EdgeStatus> {
  ...
  try {
    ...
    return {
      phase: "NO_HUB_FOUND",
      detail: `${candidate.host}:${String(candidate.port)} did not complete a mutual-TLS handshake (${
  ...
  const verdict: DiscoveryVerdict = checkRecord(
    discovery.body,
    {
      ...options.expectation,
      environment: options.environment,
  ...
  const read = async (name: string, path: string): Promise<string> => {
    try {
      const response = await call({ ...pinned, method: "GET", path });
      if (response.status === 200) return "ok";
      const result = refusalResult(response.body);
      if (result === "TERMINAL_NOT_RECOGNIZED") notRecognized = true;
  ...
  if (reads.eligibility.endsWith("PAIRING_REQUIRED")) {
    ...
    if (profileCode === undefined) {
      return {
        ...
        detail:
          "the Store Hub requires pairing, but this terminal holds no assigned profile to " +
  ...
  if (notRecognized) {
    return {
      ...
      detail:
        "the Store Hub does not hold a projection for this terminal; the cloud has not " +
  ...
  const onlyDependencies =
    failed.length > 0 && failed.every(([, value]) => value.startsWith("503 "));

  const phase: EdgePhase =
    failed.length === 0 ? "SERVING" : onlyDependencies ? "DEGRADED" : "HUB_REFUSED";
