...
export interface LinkState {
  ...
  /** The interface exists on this board at all. A Pi 5 has both; a CM might not. */
  readonly present: boolean;
  /** Layer 1. For Ethernet this is "a cable is plugged into something". */
  readonly carrier: boolean;
  /** systemd-networkd's view: `routable` is the one that means usable. */
  readonly operstate: string;
}

export interface NetworkStatus {
  readonly wired: LinkState;
  readonly wireless: LinkState;
  /** True when SOME link is routable. Says nothing about the internet. */
  readonly hasLink: boolean;
  /** A Wi-Fi configuration exists on this device. Says nothing about the PSK. */
  readonly wirelessConfigured: boolean;
}

export interface AccessPoint {
  /** Safe to print: control bytes replaced. NEVER used as the configured SSID. */
  readonly ssid: string;
  /** The SSID's REAL bytes, as the radio broadcast them. This is what is stored. */
  readonly ssidBytes: Buffer;
  /** dBm as reported by the radio. Higher (less negative) is stronger. */
  readonly signal: number;
  readonly secured: boolean;
}

/** Result of an association attempt. Never carries the passphrase. */
export type WifiJoinOutcome =
  | { readonly kind: "joined" }
  | { readonly kind: "refused"; readonly reason: string }
  | { readonly kind: "unavailable"; readonly reason: string };

type Runner = (file: string, args: readonly string[], stdin?: string) => Promise<string>;
 ...
 */
const runProcess: Runner = (file, args, stdin) =>
...
export function readLink(name: string, root = "/sys/class/net"): LinkState {
 ...
 */
export function hasSavedNetwork(configPath = WPA_CONFIG_PATH): boolean {
  try {
...
export function parseScanResults(raw: string): AccessPoint[] {
  ...
  for (const line of raw.split("\n").slice(1)) {
    ...
    if (ssidBytes.length === 0 || ssidBytes.length > SSID_MAX_BYTES) continue;
    ...
    if (existing === undefined || point.signal > existing.signal) seen.set(ssid, point);
  }
  return [...seen.values()].sort((a, b) => b.signal - a.signal);
}

export async function scanAccessPoints(run: Runner = runProcess): Promise<AccessPoint[]> {
 ...
 */
export function decodeScanSsid(encoded: string): Buffer {
  const out: number[] = [];
 ...
 */
export function displaySsid(raw: Buffer): string {
...
export function ssidToHex(ssid: string | Buffer): string {
 ...
 */
export async function derivePsk(
  ssid: string,
  passphrase: string,
...
export async function joinNetwork(
  input: { readonly ssid: string | Buffer; readonly passphrase: string },
  options?: { readonly run?: Runner; readonly configPath?: string },
 ...
 */
export function withoutNetwork(config: string, ssidHex: string): string {
 ...
 */
export async function waitForWirelessCarrier(
  timeoutMs = 25_000,
  options?: { readonly sysfsRoot?: string; readonly now?: () => number },
): Promise<boolean> {
  const now = options?.now ?? Date.now;
  const deadline = now() + timeoutMs;
  for (;;) {
    if (readLink(WIRELESS_INTERFACE, options?.sysfsRoot).carrier) return true;
 ...
 * at the base path.
 */
export async function isCloudReachable(baseUrl: string, timeoutMs = 8000): Promise<boolean> {
