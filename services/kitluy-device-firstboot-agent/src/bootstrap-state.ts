...
/**
 * Ordered because the surface renders progress, not because transitions are
 * automatic. Nothing advances a device except evidence.
 */
...
export interface BootstrapState {
  ...
  /** Why the device is where it is. Shown to an operator; never a secret. */
  readonly detail?: string;
  readonly identityReady: boolean;
  readonly networkReady: boolean;
  /** Server-issued. Absent until the cloud has actually said so. */
  readonly deviceRecordId?: string;
  readonly imageVersion?: string;
  readonly agentVersion: string;
  readonly updatedAt: string;
  /**
   * A short, non-secret device label for an operator to read aloud during
   * support. Derived from the PUBLIC key fingerprint, never from the private
   * key and never from a hardware serial (which is evidence, not identity).
   */
  readonly deviceLabel?: string;
}

/** Same atomic discipline as the identity store: temp -> fsync -> rename -> fsync dir. */
export function writeBootstrapState(state: BootstrapState, path = BOOTSTRAP_STATE_PATH): void {
...
export function readBootstrapState(path = BOOTSTRAP_STATE_PATH): BootstrapState | null {
  try {
 ...
 */
export function hasDefaultRoute(procRoot = "/proc"): boolean {
  try {
...
export function deviceLabelFromPublicKey(publicKeyPem: string): string {
