...
import type { SignedReleaseManifestBody } from "./release-verify.js";
...
export interface ArtifactSource {
  /**
   * Bytes `[offset, offset + maxBytes)` of this release's artifact.
   * `null` means "no more bytes" — end of artifact.
   */
  fetchChunk(releaseId: string, offset: number, maxBytes: number): Promise<Uint8Array | null>;
  describe(): string;
}
...
export type ArtifactResult =
  ...
  | { readonly ok: false; readonly refusal: ArtifactRefusal; readonly detail: string };
...
export async function downloadAndVerify(
  ...
  options: {
    ...
    /** Bytes already held from an interrupted attempt, if any. */
    readonly resumeFrom?: Buffer;
    /** Called after each chunk so the caller can persist a durable offset. */
