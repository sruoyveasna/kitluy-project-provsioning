 ...
 */
import { X509Certificate, createPublicKey, createHash, type KeyObject } from "node:crypto";

/** Every check, named, so a refusal says which one. */
...
const sha256Hex = (data: Buffer | Uint8Array): string =>
 ...
 */
function assertsCertificateAuthority(certificateDer: Buffer): boolean {
  const readTlv = (
    buffer: Buffer,
    offset: number,
  ): { tag: number; header: number; length: number } | null => {
  ...
  let extensionsBody: Buffer | null = null;
  while (cursor < tbsEnd) {
    const field = readTlv(certificateDer, cursor);
    if (field === null) return false;
    if (field.tag === 0xa3) {
      extensionsBody = certificateDer.subarray(
        cursor + field.header,
  ...
  if (extensionsBody === null) return false;
  ...
  while (position < end) {
    ...
    const body = extensionsBody.subarray(
      position + extension.header,
    ...
    if (body.subarray(0, BASIC_CONSTRAINTS.length).equals(BASIC_CONSTRAINTS)) {
      ...
      if (field === null || field.tag !== 0x04) return false;
      ...
      if (first === null || first.tag !== 0x01) return false;
 ...
 */
export function verifyOperationalCertificate(input: VerificationInput): VerificationOutcome {
  const failures: VerificationFailure[] = [];
  const fail = (check: VerificationCheck, detail: string): void => {
  ...
  let intermediate: X509Certificate | null = null;
  ...
  if (root !== null) {
    ...
    if (rootSha !== input.expectedRootSha256.toLowerCase()) {
      fail(
        "ROOT_IS_THE_EXPECTED_DEVELOPMENT_ROOT",
        `the chain root is ${rootSha.slice(0, 16)}…, expected ${input.expectedRootSha256.slice(0, 16)}…`,
      );
    }
  }
  ...
  let leafSpkiDer: Buffer;
  try {
    leafSpkiDer = Buffer.from(leaf.publicKey.export({ type: "spki", format: "der" }));
    const localSpkiDer = Buffer.from(
      createPublicKey(input.localPublicKeyPem).export({ type: "spki", format: "der" }),
    );
  ...
  if (leafSpkiDer.length > 0) {
    ...
    if (fingerprint !== input.expectedPublicKeyFingerprint.toLowerCase()) {
      fail(
        "SPKI_FINGERPRINT_MATCHES_GOVERNED",
        `the leaf SPKI fingerprint is ${fingerprint.slice(0, 16)}…, governed value is ${input.expectedPublicKeyFingerprint.slice(0, 16)}…`,
      );
    }
  }
  ...
  try {
    ...
    if (asInteger(leaf.serialNumber) !== asInteger(input.expectedX509Serial)) {
      fail(
        "SERIAL_MATCHES_GOVERNED",
        `the certificate serial is ${leaf.serialNumber}, governed value is ${input.expectedX509Serial}`,
      );
    }
  } catch {
  ...
  if (Number.isNaN(notBefore.getTime()) || Number.isNaN(notAfter.getTime())) {
    ...
    fail(
      "VALIDITY_WINDOW_IS_SANE",
      `the validity window is longer than ${MAX_VALIDITY_DAYS} days; this profile issues 30`,
    );
  }
  ...
  if (!Number.isNaN(notBefore.getTime()) && !Number.isNaN(notAfter.getTime())) {
    if (input.trustedTime.getTime() + NOT_BEFORE_SKEW_MS < notBefore.getTime()) {
      fail(
        "CURRENTLY_USABLE_AT_TRUSTED_TIME",
        `the certificate is not valid until ${notBefore.toISOString()}, which is more than ` +
  ...
  if (!uris.includes(`kitluy-generation://${String(input.expectedCredentialGeneration)}`)) {
    fail(
      "CREDENTIAL_GENERATION_MATCHES",
      `no kitluy-generation://${input.expectedCredentialGeneration} SAN`,
    );
  }
  ...
  if (input.expectedAssignmentGeneration < 1) {
    fail(
      "ASSIGNMENT_GENERATION_MATCHES",
      `the request carried assignment generation ${input.expectedAssignmentGeneration}`,
    );
  }
  ...
  if (!eku.includes(SERVER_AUTH) || !eku.includes(CLIENT_AUTH)) {
    fail(
      "KEY_USAGE_PROFILE_MATCHES",
      `extended key usage is [${eku.join(", ")}]; the Store Hub profile needs serverAuth and clientAuth`,
    );
  }
  ...
  if (leaf.publicKey.asymmetricKeyType !== "rsa") {
    ...
    fail(
      "ALGORITHM_IS_SUPPORTED",
      `the leaf key is RSA-${leaf.publicKey.asymmetricKeyDetails?.modulusLength ?? "unknown"}`,
    );
  }
  if (input.expectedAlgorithm !== SUPPORTED_KEY_ALGORITHM) {
    fail(
      "ALGORITHM_IS_SUPPORTED",
      `the governed response names ${input.expectedAlgorithm}; this profile supports ${SUPPORTED_KEY_ALGORITHM}`,
    );
  }
  ...
  if (!der.includes(Buffer.from("300d06092a864886f70d01010b0500", "hex"))) {
    fail(
      "ALGORITHM_IS_SUPPORTED",
      `the certificate does not declare ${SUPPORTED_SIGNATURE_ALGORITHM} with canonical parameters`,
    );
  }
