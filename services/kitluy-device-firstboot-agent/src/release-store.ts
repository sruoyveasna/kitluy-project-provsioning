 ...
 */
export const PERMITTED_PRODUCTS: readonly string[] = [
  U1_PERMITTED_PRODUCT,
  TERMINAL_CLIENT_PRODUCT,
];

export type PermittedProduct = typeof U1_PERMITTED_PRODUCT | typeof TERMINAL_CLIENT_PRODUCT;

export function isPermittedProduct(product: string): product is PermittedProduct {
...
export class ProductNotPermittedError extends Error {
  constructor(readonly product: string) {
    super(
      `PRODUCT_NOT_PERMITTED: ${product} is not an updatable product. The release store holds the Device Shell payload (OD-U1-2 = C) and the POS application ${TERMINAL_CLIENT_PRODUCT} (KLD-2026-08-11-DEVICE-BOOTSTRAP-RUNTIME-001); the bootstrap/runtime set stays image-only until the owner rules otherwise.`,
    );
...
export function assertProductPermitted(product: string): void {
  if (!isPermittedProduct(product)) throw new ProductNotPermittedError(product);
}
