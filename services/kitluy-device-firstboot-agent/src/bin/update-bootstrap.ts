 ...
 * problem.
 */
export function productsOnThisImage(
  unitDirectories: readonly string[] = UNIT_DIRECTORIES,
): readonly PermittedProduct[] {
  return PERMITTED_PRODUCTS.filter(isPermittedProduct).filter((product) =>
    unitDirectories.some((dir) => existsSync(join(dir, RELEASE_PRODUCTS[product].unit))),
  );
}

export type UpdateState =
  | { readonly kind: "no_trust_anchor"; readonly detail: string }
  | { readonly kind: "no_release_source"; readonly detail: string }
  | {
      readonly kind: "ready";
      readonly imageVersion: string;
      readonly source: string;
      /** Which file the source came from: the baked default or the override. */
      readonly sourceFrom: "image" | "override";
    };
...
function readReleaseSource(path: string): string | undefined {
  try {
...
export function evaluatePreconditions(options: {
  ...
  readonly releaseConfigPath?: string;
  readonly releaseSourceOverridePath?: string;
  readonly imageEnvPath?: string;
}): UpdateState {
  const trustDir = options.trustDir ?? TRUST_ANCHOR_DIR;
  let anchors: string[] = [];
  try {
    anchors = readdirSync(trustDir).filter(
      (name) => name.endsWith(".pub") || name.endsWith(".pem") || name.endsWith(".json"),
    );
  } catch {
    anchors = [];
  }
  ...
  const override = readReleaseSource(
    options.releaseSourceOverridePath ?? RELEASE_SOURCE_OVERRIDE_PATH,
  );
...
function readImageVersion(path: string): string | undefined {
  try {
...
export function reportOnce(
  options: {
    ...
    readonly releaseConfigPath?: string;
    readonly imageEnvPath?: string;
    /** `/etc` root, for `readImageEnv` — which resolves the file itself. */
    readonly etcRoot?: string;
    readonly storeRoot?: string;
    /** Overrides EVERY product's witness path; tests only. */
...
export async function runOnce(
  ...
  if (productsOnThisImage(options.unitDirectories).includes(TERMINAL_CLIENT_PRODUCT)) {
    ...
    if (started.action !== "NOT_NEEDED") emit("kitluy.update.terminal-client", { ...started });
  }
}

/** `systemctl`, never a shell. Returns trimmed stdout, or the failure text. */
export type SystemctlRunner = (args: readonly string[]) => { ok: boolean; output: string };
...
export function resetTerminalClientStartForTests(): void {
  terminalClientStartAttempted = false;
}

export type TerminalClientStart =
  | { readonly action: "NOT_NEEDED"; readonly detail: string }
  | { readonly action: "STARTED"; readonly releaseId: string }
  | { readonly action: "START_FAILED"; readonly releaseId: string; readonly detail: string };
 ...
 */
export function startInstalledTerminalClientOnce(
  options: { readonly storeRoot?: string; readonly systemctl?: SystemctlRunner } = {},
): TerminalClientStart {
  ...
  if (releaseId === null) return { action: "NOT_NEEDED", detail: "no POS release is installed" };
...
function describeOutcome(
  result: Awaited<ReturnType<typeof runInstallPass>>,
  ...
  if (result.outcome === "REFUSED") return { code: result.code, detail: result.detail };
...
if (process.argv[1] !== undefined && process.argv[1].includes("update-bootstrap")) void main();
