...
export type UpdateState =
  ...
  | { readonly kind: "up_to_date"; readonly imageVersion: string };
...
export async function main(): Promise<void> {
  for (;;) {
    ...
    process.stdout.write(
      `event=kitluy.update.bootstrap state=${state.kind} agent=${SERVICE_VERSION} detail=${JSON.stringify(detail)}\n`,
    );
