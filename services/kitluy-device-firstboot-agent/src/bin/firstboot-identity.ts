 ...
 */
import { bootstrapIdentity, type FirstbootDeps } from "../identity.js";
import {
  CorruptIdentityRecordError,
  DEFAULT_IDENTITY_DIR,
  FileIdentityStore,
} from "../adapters/device-identity-store.js";
...
export function buildDeps(directory: string): FirstbootDeps {
  return {
 ...
 */
function report(fields: Record<string, string | number>): void {
...
export async function main(): Promise<number> {
  ...
  try {
    ...
    const signalCount = Object.values(outcome.identity.hardwareSignals).filter(
      (v) => v !== undefined,
    ).length;
