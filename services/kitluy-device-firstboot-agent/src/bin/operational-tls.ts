...
function readRootPin(): string | null {
  try {
 ...
 */
export function readOperationalBaseUrl(etcRoot?: string): string | undefined {
  return readImageEnv("KITLUY_ENROLLMENT_BASE_URL", etcRoot);
}

async function once(): Promise<"done" | "waiting" | "blocked"> {
  const phase = currentPhase();
  if (phase === "ADOPTED") {
    const manifest = readManifest();
    log(
      `already adopted: credential ${manifest?.credentialId ?? "?"} generation ` +
  ...
  switch (outcome.kind) {
    ...
    case "adopted":
      ...
      log(
        `ADOPTED: credential ${outcome.manifest.credentialId} generation ` +
...
async function main(): Promise<void> {
  for (;;) {
    let result: "done" | "waiting" | "blocked";
    try {
      result = await once();
    } catch (error) {
