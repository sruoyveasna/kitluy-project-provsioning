...
/**
 * The local credential lifecycle.
 *
 * Names chosen to match the phases the owner instruction lists. They describe
 * what is TRUE ON DISK and authorize nothing — the cloud remains the authority
 * for the credential itself, exactly as `registration-state.ts` records for
 * registration.
 */
export type OperationalCredentialPhase =
  | "ABSENT"
  | "KEY_READY"
  | "REQUEST_READY"
  | "ISSUED_UNVERIFIED"
...
/**
 * The request identity that must survive a reboot, and nothing else.
 *
 * No private key, no certificate bytes. Every field here is either public or a
 * random identifier, so this file is safe to read in support.
 */
export interface OperationalRequestState {
  readonly phase: Extract<OperationalCredentialPhase, "REQUEST_READY">;
  ...
  /** ISO-8601. The value that entered the signature, byte for byte. */
  readonly requestedAt: string;
  readonly nonce: string;
  readonly correlationId: string;
  /** When this request was first written locally. Diagnostics only. */
  readonly createdAt: string;
}

/** What the Hub holds once adoption completed. Public metadata only. */
export interface OperationalCredentialManifest {
  readonly phase: Extract<OperationalCredentialPhase, "ADOPTED">;
  readonly deviceRecordId: string;
  readonly environment: string;
  readonly credentialId: string;
  readonly certificateGeneration: number;
  readonly certificateSerial: string;
  readonly certificateX509Serial: string;
  readonly certificateSha256: string;
  readonly publicKeyFingerprint: string;
  readonly publicKeyAlgorithm: string;
  readonly notBefore: string;
  readonly notAfter: string;
  /** Which request produced it, so a replay can be recognised. */
  readonly requestId: string;
  readonly adoptedAt: string;
}

/** temp -> fsync -> rename -> fsync(dir). The store convention, unchanged. */
function writeAtomic(path: string, contents: string, mode = FILE_MODE): void {
...
export function writeRequestState(
  state: OperationalRequestState,
 ...
 */
export function readRequestState(paths = OPERATIONAL_PATHS): OperationalRequestState | null {
  try {
    const parsed: unknown = JSON.parse(readFileSync(paths.requestState, "utf8"));
    if (parsed === null || typeof parsed !== "object") return null;
    const candidate = parsed as Partial<OperationalRequestState>;
    ...
    const required: ReadonlyArray<keyof OperationalRequestState> = [
      "requestId",
      "deviceRecordId",
      "environment",
    ...
    if (typeof candidate.assignmentGeneration !== "number") return null;
    return parsed as OperationalRequestState;
  } catch {
    return null;
  }
}

export function readManifest(paths = OPERATIONAL_PATHS): OperationalCredentialManifest | null {
  try {
    const parsed: unknown = JSON.parse(readFileSync(paths.manifest, "utf8"));
    if (parsed === null || typeof parsed !== "object") return null;
    const candidate = parsed as Partial<OperationalCredentialManifest>;
    if (candidate.phase !== "ADOPTED") return null;
    if (typeof candidate.credentialId !== "string" || candidate.credentialId === "") return null;
    if (typeof candidate.certificateSha256 !== "string") return null;
    return parsed as OperationalCredentialManifest;
  } catch {
    return null;
  }
}
 ...
 */
export function currentPhase(paths = OPERATIONAL_PATHS): OperationalCredentialPhase {
  ...
  if (readRequestState(paths) !== null) return "REQUEST_READY";
  if (existsSync(paths.privateKey)) return "KEY_READY";
  return "ABSENT";
}
