...
import type { edgeRequest } from "./edge-transport.js";
...
export type PairingOutcome =
  | { readonly paired: true; readonly profileCode: string; readonly receiptId?: string }
  | { readonly paired: false; readonly result: string; readonly detail: string };
...
function isChallenge(value: unknown): value is { readonly session: Challenge } {
  const session = (value as { session?: unknown } | null)?.session as Challenge | undefined;
  return (
...
function resultOf(body: unknown, fallback: string): string {
...
export function signPairingPayload(
  signingPayloadBase64Url: string,
  keyPath: string = DEVICE_IDENTITY_KEY_PATH,
): { readonly signature: string; readonly publicKeyPem: string } {
...
export async function pairWithHub(input: {
  readonly call: typeof edgeRequest;
  readonly base: {
    readonly host: string;
    readonly port: number;
    readonly credentials: Parameters<typeof edgeRequest>[0]["credentials"];
  ...
  try {
    ...
    return {
      ...
      detail: `cannot sign with the device identity key (${
  ...
  const pairing = (completed.body as { pairing?: { receiptId?: string } } | null)?.pairing;
  return {
    paired: true,
