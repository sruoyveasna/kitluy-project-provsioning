...
describe("the closed route list", () => {
  it.each([
    ...
    ["POST", "/edge/v1/terminal-pin/setup"],
    ["POST", "/edge/v1/terminal-pin/unlock"],
    ["POST", "/edge/v1/terminal-pin/change"],
    ["POST", "/edge/v1/terminal-pin/lock"],
  ...
  it.each([
    ...
    // No staff login on a Pi Terminal (KLD-2026-09-17-TERMINAL-PIN-DEVICE-CREDENTIAL-001):
    // the staff session door is not reachable from the board at all.
    ["POST", "/edge/v1/sessions/open"],
    ["POST", "/edge/v1/sessions/refresh"],
    ["POST", "/edge/v1/sessions/close"],
    ["GET", "/edge/v1/terminal-pin/unlock"],
