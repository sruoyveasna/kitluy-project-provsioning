...
import { resolveDeviceRevocationService, type DeviceRevocationRuntime } from "../src/composition.js";
...
describe.skipIf(!live)("one credential, from issuance to an irreversible decided revocation", () => {
  ...
  it(stage(3, "the first responder holds the emergency permission, the approver does not"), async () => {
    ...
    expect(await held(firstResponder.userId, "fleet.device_credential.emergency_revoke")).toBe(true);
  ...
  it(stage(11, "the authorization names the human and carries a database-derived digest"), async () => {
  ...
  it(stage(14, "the STATUS route reports the same obligation to an operator"), async () => {
    const response = await call({ method: "GET", path: `${EMERGENCY_PREFIX}/${authorizationId}` });
  ...
  it(stage(15, "an identical retry returns the SAME authorization, not a second one"), async () => {
  ...
  it(stage(17, "the revocation record itself is append-only to every runtime identity"), async () => {
  ...
  it(stage(18, "a signed snapshot produced NOW lists the revoked serial"), async () => {
    ...
    const producer = createSnapshotProducer(
      ...
      createEd25519SnapshotSigner({
        env: { [KEY_ENV_VAR]: pair.privateKey.export({ type: "pkcs8", format: "pem" }).toString() },
      }),
    );
