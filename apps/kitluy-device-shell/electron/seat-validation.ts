...
export interface GrantedSeat {
  readonly vertical: unknown;
  readonly terminalProfileKeys: unknown;
  readonly desiredApplications: unknown;
  readonly allowedSurfaces: unknown;
}

export interface ValidatedSeat {
...
export type SeatRefusal =
  | "SEAT_VERTICAL_MISSING"
  | "SEAT_VERTICAL_MALFORMED"
  | "SEAT_PROFILES_MISSING"
  | "SEAT_PROFILE_MALFORMED"
  | "SEAT_PROFILE_OTHER_VERTICAL"
  | "SEAT_APPLICATIONS_MISSING"
  | "SEAT_APPLICATION_MALFORMED"
  | "SEAT_APPLICATION_OTHER_VERTICAL"
  | "SEAT_SURFACES_MALFORMED";

export type SeatValidation =
  | { readonly ok: true; readonly seat: ValidatedSeat }
  | { readonly ok: false; readonly refusal: SeatRefusal; readonly detail: string };
 ...
 */
export function validateGrantedSeat(seat: GrantedSeat): SeatValidation {
  const refuse = (refusal: SeatRefusal, detail: string): SeatValidation => ({ ok: false, refusal, detail });
