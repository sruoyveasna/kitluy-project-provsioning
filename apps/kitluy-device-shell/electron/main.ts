...
import { submitPairingCode, type PairingTransport } from "./pairing.js";
...
const AGENT_PAIRING_CLIENT =
  "/usr/lib/kitluy/lib/firstboot-agent/adapters/http-terminal-pairing-client.js";
 ...
 */
async function registryBaseUrl(): Promise<string | null> {
  try {
...
async function loadTransport(): Promise<PairingTransport | null> {
  const base = await registryBaseUrl();
  if (base === null) return null;
  try {
    const loaded = (await import(pathToFileURL(AGENT_PAIRING_CLIENT).href)) as {
      createHttpTerminalPairingClient?: (o: { registryBaseUrl: string }) => PairingTransport;
    };
...
void app.whenReady().then(() => {
  ...
  ipcMain.handle(SUBMIT_CODE_CHANNEL, async (_event, code: unknown) => {
    ...
    const outcome = await submitPairingCode(code as string, {
      transport,
    ...
    if (outcome.status === "PAIRED") void requestDeviceConfig({ verb: "update.check" });
    return outcome;
  });
  ...
  ipcMain.handle(DEVICE_PIN_SETUP_CHANNEL, async (_event, pin: unknown, confirmation: unknown) => {
    ...
    if (result.ok) void requestDeviceConfig({ verb: "update.check" });
    return result;
  });
  ...
  const hex = (value: unknown): string | null =>
    typeof value === "string" && value !== "" && value.length <= 64 && /^[0-9a-fA-F]+$/.test(value)
      ? value
      : null;
  ...
  ipcMain.handle(NETWORK_JOIN_CHANNEL, async (_event, ssidHex: unknown, passphrase: unknown) => {
  ...
  ipcMain.handle(NETWORK_FORGET_CHANNEL, async (_event, ssidHex: unknown) => {
    const ssid = hex(ssidHex);
    if (ssid === null) {
      return { ok: false, code: "MALFORMED", message: "That network could not be identified." };
    }
  ...
  ipcMain.handle(BRIGHTNESS_GET_CHANNEL, () =>
    requestDeviceConfig({ verb: "display.getBrightness" }),
  );
  ipcMain.handle(BRIGHTNESS_SET_CHANNEL, async (_event, percent: unknown) => {
  ...
  ipcMain.handle(PRINTER_SAVE_CHANNEL, async (_event, config: unknown) => {
