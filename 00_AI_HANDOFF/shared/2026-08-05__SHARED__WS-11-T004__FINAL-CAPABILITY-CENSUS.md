...
| 33  | Pairing receipt                   | **Hub 0031** append-only Hub-signed receipt (§9); **P04B** `GET .../{id}/receipt`; **P04C2** terminal-local store; **P04C3** replication | receipt verify/tamper/transplant units; LAN scenario E; terminal persistence 14/14; replication 7/7 + cloud ingestion 9/9              | dev         | IMPLEMENTED-IN-DEV     | —                                                                                                                                                                                       |
...
| 36  | Pairing-receipt replication       | **Hub** `pairing-replication.ts` (outbox event in the pairing transaction); **cloud 0176** fleet projection + ingestion door             | Hub replication 7/7 (atomic, rollback, offline, restart, payload census); cloud ingestion 9/9 (idempotence, conflict, scope, boundary) | dev         | IMPLEMENTED-IN-DEV     | the WAN SENDER is WS-10; a signed Hub-to-cloud transport needs the Hub service identity (**BLK-006**)                                                                                   |
| 37  | Terminal-local encrypted store    | **`@kitluy/terminal-local-store`** — driver port + Electron `node:sqlite`, OS-protected key custody, AES-256-GCM sealing, blind indexes  | terminal persistence 14/14 (restart, wrong key, tamper, atomicity, immutability, custody, AAD binding, plaintext census)               | dev         | IMPLEMENTED-IN-DEV     | whole-file encryption (SQLCipher) is deployment material under **BLK-005**; no React Native implementation (§18, deliberate)                                                            |

## T004 CLOSEOUT VERDICT (2026-08-05, WS-11-T004 final closeout)

**WS-11-T004 is COMPLETE — IMPLEMENTED-IN-DEV.** Every capability the T004
completion criteria name is built and backed by an executed test. Pilot and
production remain BLOCKED BY BLK-005, which the criteria explicitly do not
require for `IMPLEMENTED-IN-DEV`.

Closeout evidence — one serialized run on Node v22.23.0 / pnpm 9.15.9:

| Gate                                              | Result                                                                                                                 |
| ------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| cloud `migrations:validate` + `db:validate`       | PASS — 75 files                                                                                                        |
| cloud reset from 0000 → 0176 + seed twice         | PASS — the second seed is idempotent (80 zero-row inserts)                                                             |
| cloud `db:test`                                   | **229 PASS**, exit 0 (= baseline)                                                                                      |
| cloud `test:rls`                                  | **131 PASS**, exit 0 (= baseline)                                                                                      |
| Hub reset from 0000 → **0034** + seed             | PASS — 35 migrations                                                                                                   |
| `hub:db:test`                                     | **37 PASS**, exit 0 (= baseline)                                                                                       |
| terminal-local database suite                     | **14/14**                                                                                                              |
| device-identity suite                             | **867/867** (37 files)                                                                                                 |
| Hub-agent suite                                   | **344 passed, 1 failed (pre-existing flake), 2 skipped**                                                               |
| device-registry suite (serial)                    | **363/363** (33 files)                                                                                                 |
| role-privilege census (direct + effective)        | PASS — see below                                                                                                       |
| grant / nonce / fault / temporary-object residue  | PASS — see below                                                                                                       |
| `pnpm secret:scan`                                | PASS — 1356 tracked files                                                                                              |
| `pnpm clock:check`                                | PASS — no prohibited clock access; 4/4 required consumers                                                              |
| targeted formatting of every file this task wrote | PASS                                                                                                                   |
| **`pnpm verify`**                                 | **11/12 PASS — only Format check fails, on the pre-existing repository-wide condition (36 files, none authored here)** |

**Security census.** Cloud: `service_role` does NOT effectively hold either
0176 door — it must ENTER `kitluy_edge_sync_service` through the NOINHERIT
`kitluy_edge_sync_gateway`; the ingestion identity holds EXECUTE and zero table
...
policy. Hub: both governors are NOLOGIN and granted to NOBODY; the runtime holds
SELECT plus EXECUTE and no INSERT/UPDATE/DELETE on the projection evidence;
`kitluy_sync_worker`, `kitluy_support_ro` and `public` reach no door. **No
inherited effective-privilege defect was found.**

**Residue census.** Zero temporary objects in any `edge_*` schema; zero
fabricated acknowledgements (the 0009 CHECK holds); **20 pairing receipts and
exactly 20 `terminal_pairing.receipt_issued` events** — the one-event-per-receipt
invariant, measured rather than asserted; the `kh1.*` and `kl1.*` namespaces stay
disjoint (168 / 76); 10 credential projections, all `projected`.

**Rebuild Test (PROJECT_HOME §8).** Performed for the T004 surface. The whole
provisioning-to-pairing chain is reconstructible from repository artifacts
alone: cloud migrations 0120–0176 and Hub migrations 0000–0034 carry every
contract, refusal family and grant boundary in-file with source citations and
prove their own boundary on apply; owner decision
`KLD-2026-08-05-TERMINAL-TRANSPORT-001` fixes the transport and route
contracts; `@kitluy/device-identity` holds every canonical byte format
(provisioning PoP, activation acknowledgment, pairing transcript and receipt,
edge discovery, credential package) with fixed field order;
`@kitluy/terminal-local-store` holds the terminal storage contract.

**Two gaps the Rebuild Test surfaced, both FIXED in this closeout:**

1. Hub **0032 was never registered** in `HUB_MIGRATION_ORDER`, so a rebuild from
   the canonical list would have silently omitted the owner-locked 300-second
   pairing clamp — and `hub-database.test.ts` had been failing since `5711466`.
2. Hub **0033 shipped without its §8 scope index** and without a
   relation-census entry, so a rebuild from zero failed two schema-contract
   assertions that its own behavioural tests could not reach. Migration **0034**
   adds the index additively (0033 keeps its journalled bytes), and the census
   now expects 63 relations.

A reset from zero now reaches 0034 and passes 37/37.

**Two conditions recorded, NOT closed by T004** (neither is a T004 completion
criterion): the repository-wide Prettier/CRLF format condition (36 files,
pre-existing, deliberately not mass-formatted), and the pre-existing Hub
migration marker debt in 0028–0030 — `hub:db:validate` fails on exactly those
three files, while 0033 and 0034 pass every check.

**One pre-existing test flake recorded, NOT fixed** (repository rule 1):
`sync-inbox.test.ts` → "deduplicates a redelivery of the SAME facts" fails
intermittently — 3 of 5 runs on this machine, then 3 of 3 passes — because its
fixture derives `providerEventId` from a truncated `uuidv7()` (roughly 44 bits
of the millisecond timestamp), so two tests in the same file landing within
about 16 ms mint the SAME dedupe triple and the first insert is already a
duplicate. It belongs to WS-10-T006 (`e2390b2`), predates every P04 package, and
no T004 capability depends on it; `pnpm verify` passed its Unit tests gate in
the closeout run. Fix when that area is next opened: give the fixture a random
suffix.
