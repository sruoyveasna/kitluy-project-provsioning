...
| update agent | a pass held for the PIN rechecks after `PIN_HOLD_RECHECK_SECONDS = 15`, not at `POLL_SECONDS = 300`: terminal-edge rewrites the edge status every 30 s, so the Hub's "PIN set" can lag the person's entry, and a five-minute "Installing KitLuy" reads as a failure | `800ecb2` |
