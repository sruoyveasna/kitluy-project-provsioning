# POS Capability Matrix — three-way (Canonical / Laundry / Suite)

**Filename:** `04_POS_CAPABILITY_MATRIX.md` · **Version:** v1.0.0 · **Date:** 2026-08-07
...
> **Updated 2026-08-07 under the POS unification owner decision.** §5 resolves
> KLDRV-CONF-003: **the canonical `apps/kitluy-pos-desktop-app` is the base**;
> both standalone applications are implementation **donors**. §6 authorizes
> repository consolidation without declaring locked WS-12 tasks complete —
> migrated units are `MIGRATED-NOT-YET-ACCEPTED`. §7 permits Phase 2 café code to
> be migrated as `REGISTERED-INACTIVE`.
>
> Consequently the "WS-12 gate" column no longer means _forbidden_; it means
> _acceptance belongs to that task_. Two units were built this cycle — see
> `06_POS_CONSOLIDATION_ARCHITECTURE.md` and `10_POS_FEATURE_DISPOSITION_REGISTER.md`.
...
## K. Vertical awareness — built this cycle

| ID    | Capability                              | Canonical (before)      | Laundry                      | Suite                  | Target                            | Action                | Status                       |
| ----- | --------------------------------------- | ----------------------- | ---------------------------- | ---------------------- | --------------------------------- | --------------------- | ---------------------------- |
| F-100 | Vertical registry (eight locked phases) | `@kitluy/shared-types`  | none                         | none                   | shared                            | **KEEP-CANONICAL**    | Pre-existing, reused         |
| F-101 | Phase gates                             | `@kitluy/feature-flags` | none                         | none                   | shared                            | **KEEP-CANONICAL**    | Pre-existing, reused         |
| F-102 | **Vertical resolver**                   | absent                  | none                         | none                   | `@kitluy/digital-store-context`   | **PROMOTE-SHARED**    | **BUILT** — 16 tests         |
| F-103 | **Vertical module contract**            | absent                  | none                         | none                   | `apps/…/src/vertical/contract.ts` | **PROMOTE-SHARED**    | **BUILT**                    |
| F-104 | **Vertical registry/host**              | absent                  | none                         | none                   | `apps/…/src/vertical/registry.ts` | **PROMOTE-SHARED**    | **BUILT** — 14 tests         |
| F-105 | Laundry module registration             | absent                  | terminal-select (superseded) | terminals (superseded) | `apps/…/src/vertical/modules.ts`  | **MOVE-TO-LAUNDRY**   | **BUILT** — T1–T4 registered |
| F-106 | Café module registration                | absent                  | none                         | café bulk              | `apps/…/src/vertical/modules.ts`  | **REGISTER-INACTIVE** | **BUILT** — boundary only    |

F-12 (terminal selection) is confirmed **SUPERSEDE**: the canonical model forbids
user-selectable terminal identity, and the vertical is now resolved from signed
configuration rather than chosen locally.
