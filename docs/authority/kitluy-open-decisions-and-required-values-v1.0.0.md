...
| KLREQ-025 | No Hub-local permission-grant projection exists. Permission keys without a `terminal_role:` constraint are satisfied only by a presented, exactly-scoped grant; an absent grant denies                                                                                                                                              | OPEN — fail-closed by design; a Hub-local grant projection needs a governed design                                                                                                  |
| KLREQ-026 | Multi-event commands require a Hub-issued event-key namespace. The canonical offline contract §2 defines only the terminal-issued `kl1.{terminal_device_uuid}.{client_sequence}` form, which cannot distinguish several events emitted by one command                                                                                | OPEN — amendment to offline contract §2 owed                                                                                                                                       |
| KLREQ-027 | Provider callbacks cannot enter the terminal command ledger: `edge_sync.command_result.terminal_device_id` is NOT NULL and no RBAC key exists for a service-originated callback. They therefore run a separate service pipeline                                                                                                      | OPEN — needs either a nullable-terminal command-result variant or a service-actor RBAC key                                                                                          |
