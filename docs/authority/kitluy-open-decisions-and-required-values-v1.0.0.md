...
| KLREQ-021 | Four sync-state vocabularies coexist: `edge_sync.delivery_state` (persisted), the Phase-1 spec Hub-level `sync_state`, the wire literal `pending_cloud_sync`, and `OutboxItemState` in code                                                                                                   | **RESOLVED 2026-07-28** by KLD-2026-07-28-001. Separate persisted and wire vocabularies with a canonical mapping. NOTE reconciliation **C26**: the approved persisted vocabulary differs from the deployed `edge_sync.delivery_state` enum on three values (`in_flight` vs `sending`, `rejected` vs `blocked`, `reconciliation_required` absent). An additive forward migration in Cycle 9 realigns it; WS-09 is unaffected because it only writes `pending` |
