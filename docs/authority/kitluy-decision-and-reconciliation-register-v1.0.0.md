...
**First hardware run, 2026-09-21 (handoff 52 §7).** The order held on the screens but the Shell's PIN never left the board, for two reasons reproduced on the hardware: the broker's RuntimeDirectory was `0750 root:root`, so the Shell's user could not reach the broker socket at all (`EACCES` — every Settings verb had the same fault since the first image); and the broker's setup call carried no `Idempotency-Key`, which the Hub's edge routes require (422). The update agent then installed the POS on its normal poll and the person created the PIN on the application's fallback face instead. Ruling reaffirmed and made deterministic in `ad68de1` + `2354af3`: the broker shares its directory and socket with the Shell's group; the setup call carries a fresh key; the FIRST install of the POS waits while the Hub says `setup_required` (`terminalPinSetupPending`); the Shell asks for `update.check` the moment the Hub confirms the PIN. (`ad68de1`'s stated cause — `ProtectSystem=strict` blocking the socket — was wrong and is withdrawn in `2354af3`.) Sequence on a fresh card is now enforced, not merely displayed: pair → PIN (Hub) → install → terminal screen.


## KLREC-2026-09-21-LOCATION-CODE-SHAPE-001 — the LOCATION_CODE in a display number is the cloud `store_locations.location_code` (RECORDED; the later, more specific owner decision applied)

**Conflict.** Appendix B (`edge_core.format_display_number` comment; `requireLocationCode` in `hub/commands/shared.ts`) bounded the code at 3-8 alphanumerics. KLD-2026-09-19-T1-REAL-OPERATIONS-001 decision 3 names `location_code (= store_locations.location_code, e.g. DEMO-PP-01)` as the value in the money contract, and decision 1 makes it the code in `KLB-{LOCATION_CODE}-{YYMMDD}-{SEQ}`. On the development Store that value is `DEMO-PP-01` (10 characters with hyphens): the first real confirm on the Hub would have refused `EDGE_REQUIRED_VALUE_MISSING`.

**Applied (slice 2, handoff 53).** The later, more specific owner decision wins for the code's SOURCE. `requireLocationCode` accepts uppercase alphanumerics and hyphens, 3-16, never starting or ending on a hyphen, and still refuses anything else; the Booking, payment and receipt numbers read `KLB-DEMO-PP-01-260921-000001` / `KLP-…` / `KLR-…`. The Appendix B text is not rewritten; an owner-approved numbering policy may replace the format later without touching identity or idempotency (the 0007 comment already says so).

## KLREC-2026-09-21-CASH-AT-INTAKE-UNDER-TERMINAL-PIN-001 — cash in full at intake rides INSIDE confirm-intake under the terminal's own T1 grant (RECORDED; decision 1 applied, the PIN surface not widened)

**Conflict.** KLD-2026-09-17-TERMINAL-PIN-DEVICE-CREDENTIAL-001 keeps "financial, custody, refund and override actions" out of what a Terminal PIN session may do (`T1_TERMINAL_PIN_PERMISSIONS`: the T1 intake surface only). KLD-2026-09-19-T1-REAL-OPERATIONS-001 decision 1 (owner: "cash in full, KHR + USD" at intake) makes the cash payment part of the ONE confirm-intake command served on the approved route whose registered permission is `laundry.bookings.create`.

**Applied (slice 2, handoff 53).** Cash at intake is recorded by `laundry.booking.confirm_from_draft` under `laundry.bookings.create` — the same shape the WS-09 library's `confirmIntake` already had for an intake deposit. The PIN surface is NOT widened: `payments.capture.cash` as a standalone command (`payments.record_cash_payment`, a balance collected later, T4 pickup payment) still refuses `EDGE_PERMISSION_DENIED` under a PIN session (tested). Refunds, voids and overrides remain human-actor work. If the owner wants cash at intake to require a human attribution, that is a new decision.

**Also recorded here.** PROPOSED event names `payment.tender_recorded` (one fact per tender leg; 0006 gives every leg its own event) and `document.receipt_issued` (the receipt row's event), declared in `hub/effect-contract.ts` for `laundry.booking.confirm_from_draft`; registration in the event registry is owed before any release claim beyond development. Slice 2 status: IMPLEMENTED · TESTED · INTEGRATED · NOT RELEASED · HARDWARE NOT VERIFIED; committed `6cee466` on `fix/t1-grant-pick`, not yet on `dev` (handoff 53).
