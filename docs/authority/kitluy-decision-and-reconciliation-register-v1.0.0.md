...
| RC-021            | The emergency revocation path is an unapproved, unbounded revocation entry point for the same runtime identity | **OPEN — BLOCKING** | Independent re-review 2026-07-30 finding C-2, demonstrated by execution. `revoke_device_credential_emergency_v1` retains EXECUTE for `kitluy_issuance_service` (and therefore `service_role`), consults NO approval at all, revokes the ENTIRE resolved set rather than one credential, accepts a NULL device id, and takes reauthentication / declaring authority / declared-by as unverified caller assertions. The reviewer revoked THREE credentials across THREE unrelated devices in one call with zero approvals. Its SECURITY_INCIDENT / PROVIDER_COMPROMISE branch reads `revocation_recorded_scopes` by incident reference and never calls `verify_revocation_scope_binding_v1` — independently confirmed, the live function body contains no reference to it — while `record_revocation_scope_v1`, granted to the same role, gates only on two caller-supplied strings differing. Group 0145's census does not enumerate this function at all. | KLREQ-032 permits an emergency path but requires an explicit exact scope; an unapproved path revoking a whole resolved set is broader than the decision allows | `supabase/migrations/20260729180138_0138_emergency_revocation_and_scope.sql`; census omission in `20260730100145_0145_*.sql` | Open. Fix: add the emergency function to the 0145 census, route it through the governed entry point, and make its recorded-scope branch verify the Ruling 1 binding. |
| RC-022            | SECTION 46 leaves a live, APPROVED, unconsumed exploit approval committed in the development database | OPEN — MINOR | Independent re-review 2026-07-30 finding C-4, independently confirmed: the approval with payload_hash `deadbeef-not-a-scope-hash` exists as `f235e503-87ab-47eb-a04f-0c19d3b7d5b4`, status APPROVED, quorum 2 satisfied, unconsumed, naming a device that holds issued credentials. It is directly spendable and I spent it (rolled back) to confirm RC-019's C-1. A test fixture that leaves live authorization behind changes the security posture of the database it ran against. | Test fixtures must not leave spendable authority | `supabase/tests/assertions.sql` SECTION 46 | PARTIALLY REMEDIATED 2026-07-30, STILL OPEN. The SECTION 46 exploit fixture is fixed: its five wrong approvals, including the `deadbeef-not-a-scope-hash` one, are now created inside a subtransaction rolled back by a sentinel exception, and a query for `payload_hash='deadbeef-not-a-scope-hash'` after a full `db:test` returns 0 at any status. BUT THE CLASS IS WIDER THAN THE ONE FIXTURE, and an earlier claim in this session that the residue query 'returns 0' was scoped only to that hash and understated the problem. A census of `kitluy_auth.approval_requests` after a clean reset + seed + db:test still shows at least EIGHT standing APPROVED test-created approvals, including one with a BLANK payload_hash from section 44 (`reason='an approval carrying no payload hash'`) and several carrying real 64-hex hashes (`'an approval committing to another set'`, `'a different terminal entirely'`, `'a set that names a key, not a device'`, `'risk class probe'`). SEVERITY IS LOW BUT NOT NIL: group 0147 means none of them authorizes anything on the normal path — a blank hash is refused SCOPE-UNBOUND and a foreign hash SCOPE-HASH-MISMATCH — so none is spendable as the deadbeef one was. They remain standing test-created authorization records in a database whose posture the test suite should not be changing. RC-022 closes only when a post-suite census of approvals, emergency authorizations and recorded scopes returns zero reusable rows across ALL sections, not one. |
| RC-023            | The emergency-revocation authority check cannot be wired: no device/fleet permission key is registered | **PARTIALLY RESOLVED by group 0148 — keys registered; session-binding still to build** | KLD-2026-07-29-DEVICE-CREDENTIAL-REVOCATION-001 requires emergency immediate revocation to be executed by an authorized CISO or incident commander. A governed emergency entry point must therefore VERIFY that authority rather than accept it. It cannot today, verified by query: `kitluy_auth.permissions` holds **8** rows and **none** matches `fleet%` or `%device%`, so there is no `fleet.device_credential.emergency_revoke` (or equivalent) key to check. Worse for this purpose, `kitluy_auth.has_permission(permission_key, resource_type, resource_id, environment)` resolves the CURRENT SESSION identity, not a passed actor — and the emergency caller is `kitluy_issuance_service` acting on behalf of a named human, so a session-scoped check would answer the wrong question even if a key existed. The existing `declaring_authority` column is an enum value the CALLER supplies, which is precisely the RC-021 defect. | Same shape as KLREQ-031's recorded destruction note: permission keys must be registered via the governed RBAC registry before the control that depends on them can be enforced; until then the control fails closed or is unimplementable | `kitluy_auth.permissions` (8 rows, none fleet/device); `kitluy_auth.has_permission` signature; `kitluy_devices.device_credential_emergency_revocations.declaring_authority` | Open. Needs (a) the emergency permission key(s) registered through the governed RBAC registry, and (b) an actor-scoped authority check — either a `has_permission_for_actor(actor, key, ...)` variant or an authoritative emergency-authority table keyed by the named human. RC-021's scope binding can be built without this; its ACTOR AUTHORITY verification cannot. UPDATE 2026-07-30 (group 0148): the missing half is supplied. `fleet.device_credential.emergency_revoke` and `fleet.device_credential.emergency_post_approve` are registered at risk class CRITICAL, ACTIVE, resource types {device, device_credential}, environments {all}, following the registry's existing dotted grammar. TWO distinct keys, not one, so a single assignment cannot satisfy both halves of the §2.4 four-eyes control. Verified from a clean 47-migration chain: both keys present and ACTIVE, and granted to ZERO role templates — registering a key and assigning it are separate acts and this migration performs only the first, so no human and no role template receives emergency authority from it. The second mechanism question is also settled and needed no new code: `kitluy_auth.has_permission` already resolves the actor from `auth.uid()`, fails closed on a null actor, applies explicit DENY precedence and refuses a non-ACTIVE profile — so the governed RPC must simply run under the human's own session. NO actor-scoped variant will be added: a service identity passing a human's uuid would be ASSERTING the delegation rather than proving it, which is `declaring_authority` moved one column across. RC-023 stays OPEN until the governed RPC actually evaluates these keys under an authenticated session. |
| RC-024            | No sensitive-action re-authentication max-age exists, so the emergency RPC cannot verify freshness without inventing policy | **OPEN — REQUIRED OWNER VALUE, BLOCKS RC-021/RC-023 COMPLETION** | KLD-2026-07-29-DEVICE-CREDENTIAL-REVOCATION-001 §2.3 requires re-authentication before immediate emergency revocation. The mechanism exists and is sound: `kitluy_auth.require_reauthentication(p_max_age_seconds integer)` reads `admin_user_profiles.last_reauth_at` for `auth.uid()`, requires an ACTIVE, non-disabled profile, and returns false otherwise — session-bound and fail-closed, exactly what RC-023 needs for actor identity. WHAT IS MISSING IS THE NUMBER. The max age is a CALLER PARAMETER, so the governed emergency RPC must pass one, and a search of `docs/` found no sensitive-action re-authentication duration anywhere in repository or owner authority. For a CRITICAL, immediate, irreversible, fleet-affecting action, choosing that value is a policy decision and not an engineering one: too long and re-authentication stops meaning anything, too short and an incident responder is locked out mid-incident. A SECOND, SEPARABLE GAP: there is NO re-authentication EVIDENCE model — no `reauth`/`step_up`/`elevation` table anywhere. `last_reauth_at` is a timestamp on the profile, not an evidence row, so an evidence ID cannot be passed, and evidence cannot be bound to an action class, marked superseded or revoked, or audit-linked as the emergency contract requires. That gap is schema and CAN be built additively; the duration cannot be guessed. | Repository authority already fixes the mechanism and the actor binding; only the freshness threshold is unstated, and inventing it would be inventing security policy | `kitluy_auth.require_reauthentication`; `kitluy_auth.admin_user_profiles.last_reauth_at`; no table matching `reauth|re_auth|step_up|elevation` | Open. EXACT VALUE REQUIRED FROM THE OWNER: the maximum age, in seconds, of a re-authentication that may authorize `fleet.device_credential.emergency_revoke`, and whether `fleet.device_credential.emergency_post_approve` takes the same value or a different one. Everything else on the emergency path can then be built: the evidence model additively, the authorization record, and the session-scoped RPC. |
