# KitLuy Reporting and Export Service — Phase 1 Shared-Service Specification

| Field                             | Value                                                              |
| --------------------------------- | ------------------------------------------------------------------ |
| **Filename**                      | `kitluy-reporting-and-export-service-phase1-spec-v1.0.0.md`        |
| **Version**                       | `v1.0.0`                                                           |
| **Date**                          | 2026-07-26                                                         |
| **Service**                       | `kitluy-reporting-export-service`                                  |
| **Phase**                         | Phase 1 — Laundry shared foundation                                |
| **Owner**                         | HET / KitLuy Suite Project Owner                                   |
| **Primary runtime**               | DigitalOcean container service/worker; Kubernetes-ready            |
| **Authoritative metadata**        | Supabase PostgreSQL + RLS in schema `kitluy_reporting`             |
...
> **Mission:** Produce trustworthy, permissioned and reproducible operational reports and exports with explicit source, data-as-of, sync freshness and reconciliation state, without creating a second ledger or commercial paywall.
...
Phase 1 supports Laundry operations, Bookings, payments/deposits/refunds summaries, customers, service performance, staff/shift, consumables/inventory, capacity/issues, pickup/ready aging, audit and platform/fleet reports needed for pilot and commercial operation.

## 1.2 Service owns

- Report definition/version registry and semantic query contracts.
- Read models/materializations approved for reporting.
- Report run, parameter validation, snapshot metadata and result status.
- Export generation through File Service and expiry/retention policy.
- Schedules, delivery handoff, fairness limits and reproducibility evidence.

## 1.3 Service does not own

- Authoritative transaction/payment/inventory/finance records.
- Statutory general ledger or tax filing.
- Data correction through reports.
- Commercial gating of reporting/history/export capability.
- Arbitrary user SQL or unrestricted database dumps.
...
```mermaid
flowchart LR
  Apps[Admin / Chain / Partner] --> API[Reporting Service]
  API --> Defs[(Report definitions)]
  API --> Read[(Approved read models / replicas)]
  API --> Jobs[Report/export workers]
  Jobs --> Files[File Service / Spaces]
  Jobs --> DB[(Supabase kitluy_reporting)]
  Jobs --> NTF[Notification Service optional]
  Hub[Store Hub sync status] --> Read
```

## 2.1 Deployable units

- `kitluy-reporting-export-service-api` — stateless request/control API.
- `kitluy-reporting-export-service-worker` — durable asynchronous job processing.
- `kitluy-reporting-export-service-scheduler` — optional scheduled dispatcher; may share worker image but runs as a separate process.
- Database migrations under `supabase/migrations/` for `kitluy_reporting`.
...
The target PostgreSQL schema is `kitluy_reporting`. These table names are **SPECIFIED HERE** and remain subject to the canonical Supabase schema/data-dictionary/migration pack. They must not be treated as applied until migration evidence exists.

| Table                  | Purpose                                                                       |
| ---------------------- | ----------------------------------------------------------------------------- |
| report_definitions     | Canonical report ID, owner domain, parameters, permission, output and status. |
| report_versions        | Immutable query/semantic definition and compatibility.                        |
| report_runs            | Parameters, actor, scope, source, data_as_of, freshness and state.            |
| report_results         | Summary/result metadata; large output in File Service.                        |
| export_jobs            | Format, columns, locale, size estimate and lifecycle.                         |
| export_files           | File Service asset link, checksum, expiry and download count.                 |
| report_schedules       | Approved recurrence, recipient and next run.                                  |
| materialization_states | Read-model/view refresh watermark and reconciliation state.                   |
| report_filters         | Saved filters owned by user/team without changing definition.                 |
| delivery_records       | Notification delivery of completed export/report.                             |
| fair_use_counters      | Operational capacity controls, not commercial paywall.                        |
| validation_results     | Totals/reconciliation checks for a run/version.                               |
...
| ---------------- | --------------------------------------------- |
| QUEUED           | Run accepted.                                 |
| VALIDATING       | Permissions, params and data source checked.  |
| RUNNING          | Query/materialization active.                 |
| GENERATING       | Output formatting/file generation active.     |
| COMPLETED        | Result available with source/freshness.       |
| PARTIAL          | Completed with declared missing/stale source. |
| FAILED_RETRYABLE | Temporary infrastructure failure.             |
| FAILED_FINAL     | Invalid definition/data or retries exhausted. |
| EXPIRED          | Export file access expired.                   |
...
| ---------- | ----------------------------------------------------------- | -------------------------------------------------- |
| Management | GET `/v1/reports/definitions`                               | List authorized report definitions.                |
| Management | POST `/v1/reports/{report_id}/runs`                         | Start report run; idempotency required.            |
| Management | GET `/v1/report-runs/{id}`                                  | State, result metadata, data_as_of and validation. |
| Management | POST `/v1/report-runs/{id}/exports`                         | Generate CSV/XLSX/PDF as approved.                 |
| Management | GET `/v1/export-jobs/{id}`                                  | Export state and File Service asset reference.     |
| Management | POST `/v1/report-schedules`                                 | Create authorized schedule; bounded frequency.     |
| Management | PATCH `/v1/report-schedules/{id}`                           | Pause/update schedule.                             |
| Internal   | POST `/internal/v1/reporting/materializations/{id}/refresh` | Refresh approved read model.                       |
| Internal   | POST `/internal/v1/reporting/validate/{run_id}`             | Execute reconciliation validators.                 |
...
| ---------------------------- | ----------------- | ------------------------- | ------------------------------- |
| report.run_completed         | Reporting Service | Calling surface           | Report available.               |
| report.run_failed            | Reporting Service | Calling surface/Support   | Run failed.                     |
| report.export_completed      | Export worker     | File Service/Notification | Export asset available.         |
| report.materialization_stale | Reporting Service | Admin/Consumers           | Read model freshness degraded.  |
| report.validation_failed     | Validation worker | Finance/Inventory/Support | Totals/reconciliation mismatch. |
| report.export_expired        | Reporting Service | Consumers                 | File grant/lifecycle expired.   |
...
| ------------------------ | -------- | --------------------------------------------------------- | -------------------------------------------- |
| report.execute           | normal   | Run versioned semantic query with bounded resources.      | No arbitrary SQL.                            |
| report.export            | normal   | Generate approved format and upload through File Service. | Streaming and row limits.                    |
| report.materialize       | normal   | Refresh read model/watermark.                             | Expose stale state on failure.               |
| report.validate          | high     | Compare authoritative totals and expected invariants.     | Do not publish as authoritative when failed. |
| report.schedule-dispatch | low      | Create scheduled run.                                     | Capacity/fair-use controls.                  |
| report.expire-exports    | low      | Expire access/files per policy.                           | Preserve run metadata/checksum.              |
| report.cost-capacity     | low      | Record duration/rows/bytes for capacity planning.         | No report paywall.                           |
...
| ----------------------------- | -------------------------------- | -------------------------------------- |
| report.read                   | Run/view report                  | definition-specific tenant/store/role  |
| report.export                 | Generate/download export         | same data permission plus export grant |
| report.schedule               | Create schedule                  | authorized owner/manager               |
| report.finance.read           | Access finance-sensitive reports | explicit finance permission            |
| report.audit.read             | Access audit reports             | audit/security scope                   |
| report.definition.manage      | Create/update draft definition   | data/report engineering                |
| report.definition.publish     | Publish version                  | domain owner + independent review      |
| report.materialization.manage | Refresh/repair read model        | platform/data operations               |
...
- report run volume/latency
- queue depth/oldest age
- rows/bytes generated
- export success/failure
- File Service upload latency
- materialization freshness/lag
- validation failure count
- download grant/use
- schedule delay
- resource/cost by report definition

# 12. Failure modes and degraded behavior

| Failure                    | Required behavior                                                                |
| -------------------------- | -------------------------------------------------------------------------------- |
| Read model stale           | Return stale/partial label or fail according to report sensitivity.              |
| Store Hub sync lag         | Expose Location sync freshness and data_as_of.                                   |
| Validation mismatch        | Do not label result reconciled/authoritative; alert domain owner.                |
| Large export               | Use streaming/bounded job or reject with safe limit; never overload API process. |
| File Service unavailable   | Keep generated state retryable; no fake download link.                           |
| Replica unavailable        | Use approved source/fallback or report unavailable; no silent source switch.     |
| Schedule backlog           | Delay with visible next/actual run and capacity alert.                           |
| Permission revoked mid-run | Recheck before result/download grant and deny if no longer authorized.           |
...
| ----------- | -------------------------------------------------------------------------------- |
| KREP-QA-001 | Cross-tenant report/export is denied by API and RLS.                             |
| KREP-QA-002 | Every result includes source and data_as_of.                                     |
| KREP-QA-003 | Location sync lag is visible in report.                                          |
| KREP-QA-004 | Finance validation mismatch blocks reconciled label.                             |
| KREP-QA-005 | Export file is private and accessed through File Service grant.                  |
| KREP-QA-006 | Report/export/history remains available independent of commercial plan.          |
| KREP-QA-007 | Arbitrary SQL/filter injection is rejected.                                      |
| KREP-QA-008 | Permission revoked before download prevents access.                              |
| KREP-QA-009 | Repeated run with same snapshot/params is reproducible within defined tolerance. |
| KREP-QA-010 | Expired export cannot be downloaded but audit metadata remains.                  |
...
- Validation mismatch
- Large export failure
- File Service outage
- Report definition rollback
- Schedule backlog
- Cross-tenant access incident
- Read replica/source failure
...
2. Add schema, enums/reference values, constraints, indexes and RLS migrations for `kitluy_reporting`.
...
- `[REQUIRED: Canonical Phase 1 report catalog and owners]`
- `[REQUIRED: Authoritative read models/views and refresh policy]`
- `[REQUIRED: CSV/XLSX/PDF support decision]`
- `[REQUIRED: Per-report row/size/runtime limits]`
- `[REQUIRED: Data retention/history policy]`
- `[REQUIRED: Financial/inventory validation queries]`
- `[REQUIRED: Scheduling limits and delivery policy]`
...
| ----------- | ------------------------------------------------ | -------- |
| KREP-P1-001 | Versioned report definition registry             | P0       |
| KREP-P1-002 | Source/freshness/reconciliation labeling         | P0       |
| KREP-P1-003 | Laundry operational report runs                  | P0       |
| KREP-P1-004 | Finance/payment/inventory validation             | P0       |
| KREP-P1-005 | Private exports through File Service             | P0       |
| KREP-P1-006 | Non-paywalled report/export/history access       | P0       |
| KREP-P1-007 | Schedules and completion notifications           | P1       |
| KREP-P1-008 | Materialization operations and capacity controls | P1       |
