-- ============================================================================
-- 0217  An Admin approval made on a workstation counts as approval evidence
-- ============================================================================
-- Authority: KLD-2026-09-03-TERMINAL-PROVISIONING-001 (provisioning eligibility);
--   group 0214 owns this predicate and this restates its body verbatim with ONE
--   value added to one list. Development-environment defect found on hardware,
--   2026-09-09.
--
-- WHAT WAS WRONG
-- ----------------------------------------------------------------------------
-- `evaluate_provisioning_eligibility_v1` accepts an Admin approval in place of
-- factory QA, but only when the lifecycle event records the environment as
-- `development` or `pilot`. The Management API stamps that event with its own
-- KITLUY_ENV, and on a workstation KITLUY_ENV must be `local`: the service
-- REFUSES to start with a loopback http auth URL in any other environment, and
-- a local Supabase stack serves GoTrue over exactly such a URL. That refusal is
-- deliberate and separately tested (composition.test.ts, "REFUSES http on
-- loopback when the environment is not local"), so it is not the thing to move.
--
-- The two rules therefore could not both be satisfied. A device approved on a
-- workstation carried `environment: local`, this predicate did not count it, and
-- every terminal pairing was refused with "the code was right but this device is
-- not an approved, eligible terminal" — against an approval that plainly existed
-- and that an operator could see in the portal. It cost an evening and sent two
-- people looking for a mistyped 8-character code that was never wrong.
--
-- A Store Hub was unaffected and reached ACTIVE, because Hub activation is
-- certificate-backed and never consults this predicate. That asymmetry is why
-- the defect survived: the half of the fleet that was exercised did not use it.
--
-- WHY ADDING `local` IS SAFE
-- ----------------------------------------------------------------------------
-- `local` is a WORKSTATION. It is strictly less privileged than `development`,
-- which this list already accepts, so nothing is reachable now that was not
-- reachable before by declaring `development` instead.
--
-- What matters is what is NOT added: `production` and `disaster_recovery` still
-- require a passed factory QA execution, which is the rule this predicate exists
-- to enforce. `staging` is likewise still excluded. An Admin approval remains an
-- acceptable substitute for factory QA only where hardware provenance is not a
-- production claim.
--
-- Everything else is 0214's body, restated unchanged: the enrolment must be
-- sealed, the approval must post-date the CURRENT enrolment, open trust
-- incidents still refuse, and evidence collisions still refuse.
-- ============================================================================
