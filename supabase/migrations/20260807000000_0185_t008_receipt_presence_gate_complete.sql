-- kitluy:group:0185
-- ============================================================================
-- WS-11-T008 — completing the receipt presence gate (independent re-review
-- finding NEW-6).
--
-- Reviewer B, verifying group 0184, found the presence gate that 0184 added
-- was itself incomplete: it named thirteen parameters but omitted
-- `p_effect_key`, `p_environment` and `p_correlation_id`. The shape checks
-- for the first two used bare operators (`NULL !~ pattern` and
-- `NULL not in (...)` are both NULL, not true), and the correlation id was
-- never examined at all — so all three fell through to their NOT NULL
-- columns and escaped as raw 23502, which `mapSentinel` reports as
-- INTERNAL_ERROR and a caller will retry forever against a delivery that can
-- never succeed.
--
-- That is the SAME ungoverned-escape class NEW-2, NEW-3 and NEW-4 were each
-- raised for, and 0184 declared closed rather than carried — so it is closed
-- here rather than carried. The gate now names every field it depends on,
-- and the two shape predicates are NULL-safe.
--
-- This migration is otherwise byte-equivalent to group 0184: every identity,
-- scope, generation, class, replay and digest rule is carried forward
-- unchanged.
--
-- Authority: security test system §19 (T008 remediation protocol); groups
-- 0176/0183/0184 (the door).
-- ============================================================================
...
begin
  ...
  -- T008 NEW-6: the gate names EVERY field the rules below depend on,
  -- including the three 0184 omitted (effect key, environment, correlation
  -- id) whose bare predicates were NULL-permeable for the same reason.
  if p_receipt_id is null or p_pairing_session_id is null
     ...
     or coalesce(btrim(p_effect_key), '') = ''
  ...
  if coalesce(p_effect_key, '') !~ '^kh1\.[0-9a-fA-F-]{36}\.[0-9]{1,10}$' then
    raise exception 'KLUY-PAIRING-RECEIPT-REJECTED-SCHEMA: the effect key is not the canonical kh1 shape (KLREQ-026)'
      using errcode = 'P0001';
  end if;
  if coalesce(p_environment, '') not in ('development', 'pilot', 'production') then
...
comment on function kitluy_devices.ingest_terminal_pairing_receipt_v1(text, uuid, text, uuid, uuid, uuid, integer, text, uuid, uuid, uuid, text, text, text, text, text, text, timestamptz, uuid) is
  'Group 0185 (WS-11-T008 NEW-6) re-creation. Carries group 0183 unchanged — Hub identity, scope and generation bound in SQL, governed effect-key and digest refusals — and carries the group 0184 paired-side class check, and COMPLETES the presence gate: every field the rules depend on — including the effect key, environment and correlation id 0184 omitted — is refused with the door own schema sentinel rather than escaping as a raw not-null violation. The cloud still never authors pairing and never moves paired_at.';
...
begin
  ...
  if v_def not like '%p_correlation_id is null%'
     or v_def not like '%coalesce(btrim(p_effect_key)%'
     or v_def not like '%coalesce(btrim(p_environment)%' then
    raise exception 'KLUY-MIGRATION-0185: the presence gate is still incomplete';
  end if;
  if v_def not like '%v_terminal_class%' then
    raise exception 'KLUY-MIGRATION-0185: the paired-side class check was lost';
  end if;
  if v_def not like '%KLUY-PAIRING-RECEIPT-WRONG-SCOPE%'
     or v_def not like '%KLUY-PAIRING-RECEIPT-STALE-GENERATION%' then
    raise exception 'KLUY-MIGRATION-0185: a group 0183 refusal was lost';
  end if;
  raise notice 'KLUY-MIGRATION-0185: the receipt presence gate now covers every field the door depends on';
end
$guard$;
